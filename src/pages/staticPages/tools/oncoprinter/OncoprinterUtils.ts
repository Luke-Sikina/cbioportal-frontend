...
export type OncoprinterInputLine =
  | OncoprinterInputLineType1
  | OncoprinterInputLineType2;

export function isType2(
  inputLine: OncoprinterInputLine
): inputLine is OncoprinterInputLineType2 {
...
export function initDriverAnnotationSettings(store: OncoprinterStore) {
  return observable({
    ...
    get driversAnnotated() {
      const anySelected = this.oncoKb || this.cbioportalCount || this.hotspots;
...
export async function fetchOncoKbDataForMutations(
  ...
  const mutationsToQuery = _.chain(data)
    ...
    .filter(
      d => d.proteinPosStart !== undefined && d.proteinPosEnd !== undefined
    )
    .value();
...
export function getSampleGeneticTrackData(
  ...
  for (const inputLine of type2Lines) {
    ...
    sampleIdToData[inputLine.sampleId].push(
      makeGeneticTrackDatum_Data(inputLine, hugoGeneSymbolToGene)
    );
  }
  // add missing samples
...
export function getOncoprintData(geneToSampleData: {
  [hugoGeneSymbol: string]: {
    sampleId: string;
    data: OncoprinterGeneticTrackDatum_Data[];
  }[];
...
export function parseInput(
  ...
  | { status: "complete"; result: OncoprinterInputLine[]; error: undefined }
  ...
  try {
    const result = lines.map((line, lineIndex) => {
      ...
      if (line.length === 1) {
        ...
        let ret: Partial<OncoprinterInputLineType2> = {
          sampleId,
          hugoGeneSymbol
        };
        ...
        switch (lcType) {
          case "cna":
            if (
              ["amp", "gain", "hetloss", "homdel"].indexOf(lcAlteration) === -1
            ) {
              throw new Error(
