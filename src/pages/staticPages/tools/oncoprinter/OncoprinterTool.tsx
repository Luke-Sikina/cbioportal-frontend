...
const helpSection = (
  ...
  >
    <h4>Data format</h4>
    Each row of the data can take one of two formats, with tab-delimited
    columns:
    <br />
    <strong>(1)</strong> <code>Sample</code> only (e.g. so that percent altered
    in your data can be properly calculated by including unaltered samples).
    <br />
    <strong>(2)</strong> <code>Sample</code>, <code>Gene</code>,{" "}
    <code>Alteration (defined below)</code>, <code>Type (defined below)</code>.
    <br />
    ...
    <ol>
      ...
      <li>
        ...
        <ul>
          ...
          <li>
            Copy number alteration (CNA) - please use one of the four events
            below:
            <ul>
              <li>
      ...
      <li>
        <code>Type</code>: Definition of the alteration type. It has to be one
        of the following.
        <ul>
          <li>
            For a mutation event, please use one of the five mutation types
            below:
            <ul>
              <li>
...
> {
  ...
  @action private doSubmit(dataInput: string) {
    ...
    onMobxPromise(this.store.alteredSampleIds, (alteredUids: string[]) => {
      this.oncoprinter &&
        this.oncoprinter.oncoprint.setHorzZoomToFit(alteredUids);
    });
  }
  ...
  private onClickSubmit() {
    if (
      ...
      this.filesInput.files.length > 0
    ) {
  ...
  private getHelpSection() {
    return (
      <div>
        <span>
          Please input <strong>tab-delimited</strong> genomic alteration events.
        </span>
        <Button
          style={{ marginLeft: 7 }}
          bsStyle="primary"
    ...
    return (
      <FormGroup style={{ display: this.dataInputOpened ? undefined : "none" }}>
        <ControlLabel>
          ...
          <Button
            className="oncoprinterExampleData"
            style={{ marginLeft: 7 }}
            bsStyle="primary"
            bsSize="small"
            onClick={this.populateExampleData}
          >
            Load example data
          </Button>
        </ControlLabel>
        ...
        <Button
          ...
          bsSize="large"
          bsStyle="primary"
          disabled={this.dataInput.trim().length === 0}
      ...
      <PageLayout className={"whiteBackground staticPage"}>
        ...
        <WindowWidthBox offset={60}>
          <div className="cbioportal-frontend">
            <h1 style={{ display: "inline" }}>Oncoprinter</h1> generates
            Oncoprints from your own data.
            <br />
            <br />
            ...
            {!this.dataInputOpened && (
              <button
                ...
                style={{ paddingLeft: 50, paddingRight: 50, marginBottom: 15 }}
                onClick={() => {
            ...
            <div
              style={{
                display: this.store.hasData() && !this.error ? "block" : "none"
              }}
            >
              <MSKTabs
                ...
                >
                  {numCells > 100000 && (
                    <div className="alert alert-warning">
                      ...
                      may be huge and unresponsive. We recommend plotting data
                      for fewer genes at a time.
                    </div>
                  )}
