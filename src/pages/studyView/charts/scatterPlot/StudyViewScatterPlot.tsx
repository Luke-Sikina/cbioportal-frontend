...
@observer
export default class StudyViewScatterPlot
  extends React.Component<IStudyViewScatterPlotProps, {}>
  ...
  private onSelection(points: any) {
    const selectedSamples = _.reduce(
      points,
      function(acc, point) {
        _.each(point.data, datum =>
          _.each(datum.data, (d: IStudyViewScatterPlotData) =>
            acc.push({
              sampleId: d.sampleId,
              studyId: d.studyId
            })
  ...
  @computed get data() {
    return getDownsampledData(
      this.props.data,
      this.dataSpaceToPixelSpace,
      this.props.sampleToAnalysisGroup
    );
  }
  ...
  @computed get scatters() {
    ...
    const sortedAnalysisGroups = _.sortBy(this.props.analysisGroups, g =>
      g.value === "NA" ? 0 : 1
    );
    return _.reduce(
      ...
      (scatters, group) => {
        ...
        if (groupData && groupData.length > 0) {
          ...
          _.forEach(bySize, (data, numSamplesInDot) => {
            const fillOpacity = this.fillOpacityByNumSamplesInDot(
              parseInt(numSamplesInDot, 10)
            );
            scatters.push(
              <VictoryScatter
  ...
  private size(d: DSData<IStudyViewScatterPlotData>) {
    ...
    return Math.min(
      MAX_DOT_SIZE,
      baseSize - increment + increment * d.data.length
    );
  }
