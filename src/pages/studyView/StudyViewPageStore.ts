...
export class StudyViewPageStore {
  ...
  private async createNumberAttributeComparisonSession(
    ...
    return new Promise<string>(resolve => {
      onMobxPromise(this.selectedSamples, async selectedSamples => {
        ...
        const groups = quartiles.map(quartile => {
          ...
          if (clinicalAttribute.patientAttribute) {
            studies = getStudiesAttr(
              _.flatMapDeep(quartile, (d: ClinicalData) => {
            ...
            studies = getStudiesAttr(
              quartile.map(d => ({ studyId: d.studyId, sampleId: d.sampleId }))
            );
          }
          const range = [
            quartile[0].value,
            quartile[quartile.length - 1].value
          ];
          return {
  ...
  private createSpecialChartComparisonSession(
    ...
    return new Promise<string>(resolve => {
      onMobxPromise<any>(
        [this.selectedSamples, this.cancerStudiesData],
        async (
          selectedSamples: Sample[],
          cancerStudiesData: ClinicalDataCountSummary[]
        ) => {
          // group samples by study
          const studyIdToSamples: { [studyId: string]: Sample[] } = _.groupBy(
            selectedSamples,
            s => s.studyId
          );
          const studyIdToCountWithColor = _.keyBy(
            cancerStudiesData,
            s => s.value
          );
  ...
  private createStringAttributeComparisonSession(
    ...
    return new Promise<string>(resolve => {
      onMobxPromise(this.selectedSamples, async selectedSamples => {
        ...
        let lcValueToSampleIdentifiers: {
          [value: string]: SampleIdentifier[];
        } = {};
        if (clinicalAttribute.patientAttribute) {
        ...
        const sortedAttrVals = _.sortBy(
          clinicalAttributeValues,
          attrVal => -attrVal.count
        );
    ...
    if (isSpecialChart(params.chartMeta)) {
      ...
      switch (this.chartsType.get(params.chartMeta.uniqueKey)) {
        ...
        case ChartTypeEnum.TABLE:
          sessionId = await this.createStringAttributeComparisonSession(
  ...
  private _clinicalDataEqualityFilterSet = observable.shallowMap<
    ClinicalDataEqualityFilter
  >();
  private _clinicalDataIntervalFilterSet = observable.shallowMap<
    ClinicalDataIntervalFilter
  >();
  ...
  updateStoreByFilters(filters: Partial<StudyViewFilter>) {
    if (
      _.isArray(filters.clinicalDataEqualityFilters) &&
      filters.clinicalDataEqualityFilters.length > 0
    ) {
      _.each(
        filters.clinicalDataEqualityFilters,
        (filter: ClinicalDataEqualityFilter) => {
          this._clinicalDataEqualityFilterSet.set(
            getClinicalAttributeUniqueKeyByDataTypeAttrId(
              filter.clinicalDataType,
              filter.attributeId
            ),
            {
    ...
    if (
      _.isArray(filters.clinicalDataIntervalFilters) &&
      filters.clinicalDataIntervalFilters.length > 0
    ) {
      _.each(
        filters.clinicalDataIntervalFilters,
        (filter: ClinicalDataIntervalFilter) => {
          this._clinicalDataIntervalFilterSet.set(
            getClinicalAttributeUniqueKeyByDataTypeAttrId(
              filter.clinicalDataType,
              filter.attributeId
            ),
    ...
    if (_.isArray(filters.mutatedGenes) && filters.mutatedGenes.length > 0) {
      this._mutatedGeneFilter = _.reduce(
        filters.mutatedGenes,
        (acc, next) => {
          acc.push({
            entrezGeneIds: _.reduce(
              next.entrezGeneIds,
              (geneAcc, entrezGeneId) => {
    ...
    if (_.isArray(filters.cnaGenes) && filters.cnaGenes.length > 0) {
      this._cnaGeneFilter = _.reduce(
        filters.cnaGenes,
        (acc, next) => {
          acc.push({
            alterations: _.reduce(
              next.alterations,
              (altAcc, alt) => {
                altAcc.push({
  ...
  async updateStoreFromURL(query: StudyViewURLQuery) {
    ...
    if (query.filters) {
      try {
        filters = JSON.parse(decodeURIComponent(query.filters)) as Partial<
          StudyViewFilter
        >;
      ...
      const matchedAttr = _.find(
        ...
        (attr: ClinicalAttribute) =>
          attr.clinicalAttributeId === query.filterAttributeId
      );
          ...
          filters.clinicalDataIntervalFilters = [
            {
              ...
              clinicalDataType: matchedAttr.patientAttribute
  ...
  @autobind
  getKnownHugoGeneSymbolByEntrezGeneId(
    entrezGeneId: number
  ): string | undefined {
  ...
  toggleWithMutationDataFilter() {
    ...
    if (isSelected) {
      ...
      this.customChartFilterSet.set(UniqueKey.WITH_MUTATION_DATA, [
        Datalabel.YES
      ]);
    } else {
  ...
  updateClinicalDataEqualityFilters(chartMeta: ChartMeta, values: string[]) {
    ...
    if (values.length > 0) {
      let clinicalDataEqualityFilter = {
        ...
        clinicalDataType: getClinicalDataType(
  ...
  @autobind
  @action
  updateClinicalDataIntervalFiltersByValues(
    chartMeta: ChartMeta,
  ...
  addGeneFilters(genes: GeneIdentifier[]) {
    ...
    genes.forEach(
      gene => (this.geneMapCache[gene.entrezGeneId] = gene.hugoGeneSymbol)
    );
    this._mutatedGeneFilter = [
      ...this._mutatedGeneFilter,
  ...
  addCNAGeneFilters(filters: CopyNumberAlterationIdentifier[]) {
    ...
    filters.forEach(
      filter => (this.geneMapCache[filter.entrezGeneId] = filter.hugoGeneSymbol)
    );
    this._cnaGeneFilter = [
      ...this._cnaGeneFilter,
      {
    ...
    this._cnaGeneFilter = _.reduce(
      ...
      (acc, next) => {
        const newGroup = _.reduce(
          ...
          (list, filter) => {
            if (
              filter.entrezGeneId !== toBeRemoved.entrezGeneId &&
              filter.alteration !== toBeRemoved.alteration
            ) {
              list.push(filter);
            }
            return list;
  ...
  get filters(): StudyViewFilter {
    ...
    if (this._withMutationDataFilter !== undefined) {
      let customChartFilterSet = this.customChartFilterSet.get(
        UniqueKey.WITH_MUTATION_DATA
      );
      if (
        customChartFilterSet !== undefined &&
        customChartFilterSet.length === 1
      ) {
  ...
  public getClinicalDataIntervalFiltersByUniqueKey(
    uniqueKey: string
  ): ClinicalDataIntervalFilterValue[] {
  ...
  get unfilteredAttrsForNonNumerical() {
    const visibleNonNumericalAttributes = this.visibleAttributes.filter(
      (chartMeta: ChartMeta) => {
        if (
          ...
          const key = getClinicalAttributeUniqueKey(
    ...
    return visibleNonNumericalAttributes.map((chartMeta: ChartMeta) => {
      return {
        attributeId: chartMeta.clinicalAttribute!.clinicalAttributeId,
        clinicalDataType: chartMeta.clinicalAttribute!.patientAttribute
          ? "PATIENT"
          : "SAMPLE"
    ...
    return this.clinicalAttributes.result
      ...
      .map(attr => {
        return this._clinicalDataBinFilterSet.get(
          getClinicalAttributeUniqueKey(attr)
        )!;
      });
  }
  ...
  get unfilteredAttrsForNumerical() {
    ...
    return visibleNumericalAttributes.map((chartMeta: ChartMeta) => {
      return this._clinicalDataBinFilterSet.get(
        getClinicalAttributeUniqueKey(chartMeta.clinicalAttribute!)
      )!;
    });
  }
  ...
  readonly unfilteredClinicalDataCount = remoteData<ClinicalDataCountItem[]>({
    ...
    onResult: data => {
      data.forEach(item => {
        const uniqueKey = getClinicalAttributeUniqueKeyByDataTypeAttrId(
        ...
        if (this.isNewlyAdded(uniqueKey)) {
          this.showAsPieChart(
            uniqueKey,
            item.counts.length,
            item.clinicalDataType === "PATIENT" ? true : false
          );
  ...
  public getClinicalDataCount(chartMeta: ChartMeta) {
    let uniqueKey: string = getClinicalAttributeUniqueKey(
      chartMeta.clinicalAttribute!
    );
    if (!this.clinicalDataCountPromises.hasOwnProperty(uniqueKey)) {
      const isDefaultAttr =
        _.find(
          ...
          attr => getClinicalAttributeUniqueKey(attr) === uniqueKey
        ) !== undefined;
      ...
      >({
        ...
        invoke: async () => {
          let dataType: "SAMPLE" | "PATIENT" = chartMeta.clinicalAttribute!
            .patientAttribute
            ? "PATIENT"
            : "SAMPLE";
  ...
  public getClinicalDataBin(chartMeta: ChartMeta) {
    const uniqueKey: string = getClinicalAttributeUniqueKey(
      chartMeta.clinicalAttribute!
    );
    if (!this.clinicalDataBinPromises.hasOwnProperty(uniqueKey)) {
      ...
      this.clinicalDataBinPromises[uniqueKey] = remoteData<DataBin[]>({
        ...
        invoke: async () => {
          const clinicalDataType = chartMeta.clinicalAttribute!.patientAttribute
            ? "PATIENT"
            : "SAMPLE";
          ...
          const initDataBinFilter = _.find(
            this.initialVisibleAttributesClinicalDataBinAttributes.result,
            item =>
              getClinicalAttributeUniqueKeyByDataTypeAttrId(
                item.clinicalDataType,
                item.attributeId
          ...
          const attribute = this._clinicalDataBinFilterSet.get(
            getClinicalAttributeUniqueKey(chartMeta.clinicalAttribute!)
          )!;
          const attributeChanged =
            isDefaultAttr && !_.isEqual(toJS(attribute), initDataBinFilter);
          if (
            this.isInitialFilterState &&
            isDefaultAttr &&
            !this._clinicalDataIntervalFilterSet.has(uniqueKey) &&
            ...
            result = this.initialVisibleAttributesClinicalDataBinCountData
              .result;
          } else {
  ...
  private async getClinicalDataBySamples(samples: Sample[]) {
    ...
    _.forEach(sampleClinicalData, item => {
      clinicalData[item.uniqueSampleKey] = {
        ...(clinicalData[item.uniqueSampleKey] || {}),
        ["SAMPLE_" + item.clinicalAttributeId]: item.value
      };
    });
    ...
    _.forEach(patientClinicalData, item => {
      (patientSamplesMap[item.uniquePatientKey] || []).forEach(sample => {
        clinicalData[sample.uniqueSampleKey] = {
          ...(clinicalData[sample.uniqueSampleKey] || {}),
          ["PATIENT_" + item.clinicalAttributeId]: item.value
        };
      });
    });
    return clinicalData;
  ...
  readonly queriedPhysicalStudyIds = remoteData({
    ...
    invoke: () => {
      return Promise.resolve(
        _.map(this.queriedPhysicalStudies.result, study => study.studyId)
      );
    },
    default: []
  });
          ...
          .each((sampleIdentifiers, studyId) => {
            if (result[studyId] !== undefined) {
              if (result[studyId].length > 0) {
                ...
                const filteredSampleIds = sampleIdentifiers.map(
                  sampleIdentifier => sampleIdentifier.sampleId
                );
                result[studyId] = _.intersection(sampleIds, filteredSampleIds);
              } else {
                result[studyId] = sampleIdentifiers.map(
                  sampleIdentifier => sampleIdentifier.sampleId
                );
              }
            }
  ...
  readonly originStudies = remoteData({
    ...
    invoke: async () => {
      ...
      if (this.showOriginStudiesInSummaryDescription) {
        const originStudyIds = this.filteredVirtualStudies.result[0].data
          .origin;
  ...
  readonly mutationProfiles = remoteData({
    ...
    invoke: async () => {
      return this.molecularProfiles.result.filter(
        profile => profile.molecularAlterationType === "MUTATION_EXTENDED"
      );
    },
    onError: error => {},
    default: []
  ...
  updateCustomSelect(newChart: CustomChart) {
    const sampleIdentifiers = _.reduce(
      ...
      (acc, next) => {
        acc.push(
          ...next.sampleIdentifiers.map((customCase: CustomChartIdentifier) => {
            return {
  ...
  get chartMetaSet(): { [id: string]: ChartMeta } {
    let _chartMetaSet: { [id: string]: ChartMeta } = _.reduce(
      this._customCharts.values(),
      (acc: { [id: string]: ChartMeta }, chartMeta: ChartMeta) => {
  ...
  updateChartStats() {
    ...
    if (!_.isEmpty(this.mutationProfiles.result)) {
      const mutatedGeneMeta = _.find(
        this.chartMetaSet,
        chartMeta => chartMeta.uniqueKey === UniqueKey.MUTATED_GENES_TABLE
      );
      this.chartsType.set(
      ...
      this.chartsDimension.set(
        UniqueKey.MUTATED_GENES_TABLE,
        STUDY_VIEW_CONFIG.layout.dimensions[ChartTypeEnum.MUTATED_GENES_TABLE]
      );
    ...
    if (!_.isEmpty(this.cnaProfiles.result)) {
      const cnaGeneMeta = _.find(
        this.chartMetaSet,
        chartMeta => chartMeta.uniqueKey === UniqueKey.CNA_GENES_TABLE
      );
      this.chartsType.set(
      ...
      this.chartsDimension.set(
        UniqueKey.CNA_GENES_TABLE,
        STUDY_VIEW_CONFIG.layout.dimensions[ChartTypeEnum.CNA_GENES_TABLE]
      );
  ...
  initializeChartStatsByClinicalAttributes() {
    ...
    if (
      ...
      getDefaultPriorityByUniqueKey(UniqueKey.MUTATION_COUNT_CNA_FRACTION) !== 0
    ) {
    ...
    _.each(this.specialChartKeysInCustomCharts, key => {
      this.showAsPieChart(
        key,
        key === UniqueKey.CANCER_STUDIES
          ? this.cancerStudiesData.result.length
          : 2,
        false
      );
    });
    ...
    invoke: async () => {
      ...
      if (this.initialFilters.clinicalDataEqualityFilters !== undefined) {
        initialFilteredAttributeIds = initialFilteredAttributeIds.concat(
          this.initialFilters.clinicalDataEqualityFilters.map(
      ...
      if (this.initialFilters.clinicalDataIntervalFilters !== undefined) {
        initialFilteredAttributeIds = initialFilteredAttributeIds.concat(
          this.initialFilters.clinicalDataIntervalFilters.map(
            item => item.attributeId
          )
        );
      }
      ...
      if (initialFilteredAttributeIds.length > 0) {
        const initialFilteredAttributes = _.filter(
          this.clinicalAttributes.result,
          item => initialFilteredAttributeIds.includes(item.clinicalAttributeId)
        );
  ...
  >({
    ...
    invoke: async () => {
      return internalClient.fetchClinicalDataBinCountsUsingPOST({
        dataBinMethod: "STATIC",
        clinicalDataBinCountFilter: {
          attributes: this.initialVisibleAttributesClinicalDataBinAttributes
            .result,
          studyViewFilter: this.initialFilters
        } as ClinicalDataBinCountFilter
      });
  ...
  readonly selectedPatientKeys = remoteData<string[]>({
    ...
    invoke: async () => {
      return _.uniq(
        this.selectedSamples.result.map(sample => sample.uniquePatientKey)
      );
    },
    onError: error => {},
    default: []
        ...
        return mutatedGenes.map(item => {
          return {
            ...
            oncokbAnnotated: this.oncokbCancerGeneFilterEnabled
              ...
              : false,
            oncokbOncogene: this.oncokbCancerGeneFilterEnabled
              ? this.oncokbOncogeneEntrezGeneIds.result.includes(
                  item.entrezGeneId
                )
              : false,
            oncokbTumorSuppressorGene: this.oncokbCancerGeneFilterEnabled
  ...
  public getClinicalDataCountSummary(
    ...
    data = data.concat(
      clinicalDataCountSummaries.map(clinicalData =>
        [clinicalData.value, clinicalData.count, clinicalData.freq].join("\t")
      )
    );
    return data.join("\n");
  }
    ...
    if (chartMeta.clinicalAttribute && this.samples.result) {
      const clinicalDataList = await defaultClient.fetchClinicalDataUsingPOST({
        ...
        clinicalDataMultiStudyFilter: {
          ...
          identifiers: this.selectedSamples.result.map(sample => ({
            entityId: chartMeta.clinicalAttribute!.patientAttribute
              ? sample.patientId
              : sample.sampleId,
            studyId: sample.studyId
          }))
      ...
      data = data.concat(
        clinicalDataList.map(clinicalData => {
          const row = [
            clinicalData.studyId || NA_DATA,
            clinicalData.patientId || NA_DATA
          ];
            ...
            this.selectedPatients.map((patient: Patient) => {
              let record = _.find(
                ...
                (caseIdentifier: CustomChartIdentifierWithValue) => {
                  return (
                    caseIdentifier.studyId === patient.studyId &&
                    patient.patientId === caseIdentifier.patientId
                  );
                }
              );
              return [
                patient.studyId || NA_DATA,
                patient.patientId || NA_DATA,
          ...
          data = data.concat(
            this.selectedSamples.result!.map((sample: Sample) => {
              let record = _.find(
                ...
                (caseIdentifier: CustomChartIdentifierWithValue) => {
                  return (
                    caseIdentifier.studyId === sample.studyId &&
                    sample.sampleId === caseIdentifier.sampleId
                  );
                }
              );
              return [
                sample.studyId || NA_DATA,
                sample.patientId || NA_DATA,
                sample.sampleId || NA_DATA,
                record === undefined ? "NA" : record.value
              ].join("\t");
            })
          );
  ...
  public async getMutatedGenesDownloadData() {
    if (this.mutatedGeneData.result) {
      ...
      let data = [header.join("\t")];
      _.each(
        this.mutatedGeneData.result,
        (record: MutationCountByGeneWithCancerGene) => {
          let rowData = [
            ...
            getFrequencyStr(
              (record.numberOfAlteredCases / record.numberOfSamplesProfiled) *
                100
            )
          ];
          if (this.oncokbCancerGeneFilterEnabled) {
  ...
  public async getGenesCNADownloadData() {
    if (this.cnaGeneData.result) {
      ...
      let data = [header.join("\t")];
      _.each(
        this.cnaGeneData.result,
        (record: CopyNumberCountByGeneWithCancerGene) => {
          let rowData = [
            record.hugoGeneSymbol,
            record.qValue === undefined ? "" : getQValue(record.qValue),
            record.cytoband,
    ...
    invoke: async () => {
      if (
        !!this.clinicalAttributes.result!.find(
          a => a.clinicalAttributeId === MUTATION_COUNT
        ) &&
        !!this.clinicalAttributes.result!.find(
          a => a.clinicalAttributeId === FRACTION_GENOME_ALTERED
        )
      ) {
        ...
        const yBinSize =
          Math.max(...bins.map(bin => bin.binY)) / (yAxisBinCount - 1);
        return {
          bins,
          xBinSize,
          yBinSize
          ...
          if (data.length == 2) {
            ...
            _.forEach(data, datum => {
              if (datum.clinicalAttributeId === MUTATION_COUNT) {
                ...
                datum.clinicalAttributeId === FRACTION_GENOME_ALTERED
              ) {
  ...
  readonly getDataForClinicalDataTab = remoteData({
    ...
    invoke: async () => {
      ...
      } = await this.getClinicalDataBySamples(this.selectedSamples.result);
      return _.reduce(
        this.selectedSamples.result,
        (acc, next) => {
    ...
    invoke: async () => {
      if (!_.isEmpty(this.chartMetaSet)) {
        ...
        const ret: { [clinicalAttributeId: string]: number } = _.reduce(
          ...
          (map: ClinicalDataCountSet, next: ClinicalAttributeCount) => {
        ...
        if (UniqueKey.CANCER_STUDIES in this.chartMetaSet) {
          ret[UniqueKey.CANCER_STUDIES] = _.sumBy(
    ...
    invoke: async () => {
      if (!_.isEmpty(this.chartMetaSet)) {
        const attributeIdToAttribute = this
          .clinicalAttributeIdToClinicalAttribute.result!;
        // build map
        const ret: ClinicalDataCountSet = _.reduce(
          this.clinicalAttributesCounts.result || [],
          (map: ClinicalDataCountSet, next: ClinicalAttributeCount) => {
            const attribute = attributeIdToAttribute[next.clinicalAttributeId];
            if (attribute) {
        ...
        if (UniqueKey.MUTATED_GENES_TABLE in this.chartMetaSet) {
          ret[UniqueKey.MUTATED_GENES_TABLE] = this.molecularProfileSampleCounts
            .result
            ? this.molecularProfileSampleCounts.result
                .numberOfMutationProfiledSamples
            : 0;
          ret[UniqueKey.WITH_MUTATION_DATA] = this.molecularProfileSampleCounts
            .result
            ? this.molecularProfileSampleCounts.result
                .numberOfMutationProfiledSamples
            : 0;
        ...
        if (UniqueKey.CNA_GENES_TABLE in this.chartMetaSet) {
          ret[UniqueKey.CNA_GENES_TABLE] = this.molecularProfileSampleCounts
            .result
            ? this.molecularProfileSampleCounts.result
                .numberOfCNAProfiledSamples
            : 0;
          ret[UniqueKey.WITH_CNA_DATA] = this.molecularProfileSampleCounts
            .result
            ? this.molecularProfileSampleCounts.result
                .numberOfCNAProfiledSamples
            : 0;
  ...
  public async getDownloadDataPromise() {
    ...
    let clinicalAttributesNameSet = _.reduce(
      ...
      (acc, next) => {
        let id =
          (next.patientAttribute ? "PATIENT" : "SAMPLE") +
          "_" +
      ...
      switch (chartMeta.uniqueKey) {
        ...
        case UniqueKey.WITH_MUTATION_DATA: {
          switch (values.length) {
            case 1: {
              this._withMutationDataFilter =
                values[0] === Datalabel.YES ? true : false;
              break;
            }
            case 2: {
              this._withMutationDataFilter = undefined;
  ...
  showAsPieChart(
    ...
    if (
      shouldShowChart(this.initialFilters, dataSize, this.samples.result.length)
    ) {
      ...
      switch (uniqueKey) {
        case UniqueKey.WITH_MUTATION_DATA:
          this.customChartsPromises[
            uniqueKey
          ] = this.withMutationDataChartCounts;
          break;
        case UniqueKey.WITH_CNA_DATA:
        ...
        default:
          ...
          >({
            await: () => {
              return _.includes(
                [UniqueKey.WITH_MUTATION_DATA, UniqueKey.WITH_CNA_DATA],
                uniqueKey
              )
                ? [this.molecularProfileSampleCounts, this.selectedSamples]
                : [this.selectedSamples];
            },
            invoke: () => {
              ...
              dataCountSet = _.reduce(
                ...
                (acc, sample) => {
                  const matchedCases = _.filter(
                    this._customChartsSelectedCases.get(uniqueKey),
                    (selectedCase: CustomChartIdentifierWithValue) =>
                      selectedCase.sampleId === sample.sampleId
                  );
  ...
  private withMutationDataChartCounts = remoteData<ClinicalDataCountSummary[]>({
    invoke: async () => {
      let molecularProfileSampleCounts = await internalClient.fetchMolecularProfileSampleCountsUsingPOST(
        {
          studyViewFilter: Object.assign({}, this.filters, {
            withMutationData: undefined
          })
        }
      );
      return Promise.resolve(
