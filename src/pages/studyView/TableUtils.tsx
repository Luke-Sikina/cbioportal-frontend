...
export function getCancerGeneToggledOverlay(cancerGeneFilterEnabled: boolean) {
  if (cancerGeneFilterEnabled) {
    return (
      <span>
        Filtered by {getOncoKBCancerGeneListLinkout()}. Click to show all genes.
      </span>
    );
  } else {
    return (
      <span>
        Showing all genes. Click to filter by {getOncoKBCancerGeneListLinkout()}
        .
      </span>
    );
  }
...
export function getFreqColumnRender(
  ...
  const addTotalProfiledOverlay = () => (
    ...
    >
      <span>{`# of samples profiled for ${
        type === "mutation" ? "mutations" : "copy number alterations"
      } in this gene: ${numberOfSamplesProfiled.toLocaleString()}`}</span>
      <GenePanelList
