...
import {
  GeneIdentifier,
  MutatedGenesData,
  MutationCountByGeneWithCancerGene
...
class MutatedGenesTableComponent extends FixedHeaderTable<
  MutationCountByGeneWithCancerGene
> {}

@observer
export class MutatedGenesTable extends React.Component<
  IMutatedGenesTablePros,
  {}
> {
  @observable
  private preSelectedRows: MutatedGenesTableUserSelectionWithIndex[] = [];
  ...
  @computed
  get cellMargin() {
    const maxNumberMutationsColumn = _.max(
      this.tableData.map(item => item.totalCount)
    );
    const maxNumberColumn = _.max(
      this.tableData!.map(item => item.numberOfAlteredCases)
    );
    const localeNumberMutationsString =
      maxNumberMutationsColumn === undefined
        ? ""
        : maxNumberMutationsColumn.toLocaleString();
    const localeNumberString =
      maxNumberColumn === undefined ? "" : maxNumberColumn.toLocaleString();
    return {
      [ColumnKey.GENE]: 0,
  ...
  @computed get isFilteredByCancerGeneList() {
    return (
      !!this.props.cancerGeneFilterEnabled && this.props.filterByCancerGenes
    );
  }
    ...
    return [
      {
        ...
        headerRender: () => {
          return getGeneColumnHeaderRender(
            this.cellMargin[ColumnKey.GENE],
            ColumnKey.GENE,
      ...
      },
      {
        ...
        headerRender: () => {
          return (
            <div
              style={{
                marginLeft: this.cellMargin[ColumnKey.NUMBER_MUTATIONS]
              }}
            >
              # Mut
            </div>
      ...
      },
      {
        ...
        headerRender: () => {
          return (
            <div style={{ marginLeft: this.cellMargin[ColumnKey.NUMBER] }}>
              #
            </div>
          );
        },
      ...
      },
      {
        ...
        headerRender: () => {
          return (
            <div style={{ marginLeft: this.cellMargin[ColumnKey.FREQ] }}>
              Freq
            </div>
          );
        },
        ...
        filter: (
          ...
          return _.toString(
            getFrequencyStr(
              data.numberOfAlteredCases / data.numberOfSamplesProfiled
            )
