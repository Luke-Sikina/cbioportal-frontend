...
import { StudyViewPageTabKeyEnum } from "./StudyViewPageStore";
...
describe("StudyViewUtils", () => {
  ...
  describe("processDataBins", () => {
    ...
    it("determines log scale from an array of data bins", () => {
      ...
      assert.isTrue(
        isLogScaleByDataBins(logScaleDataBinsWithNegativeAndNaAndSpecialValues)
      );
    });
  });
  ...
  describe("getDefaultChartTypeByClinicalAttribute", () => {
    it("return TABLE when the clinical attributes are pre-defined as table", () => {
      ...
      assert.isTrue(
        getDefaultChartTypeByClinicalAttribute(attr) === ChartTypeEnum.TABLE
      );
    ...
    it("return PIE_CHART when clinical attribute has data type as STRING", () => {
      ...
      assert.isTrue(
        getDefaultChartTypeByClinicalAttribute(attr) === ChartTypeEnum.PIE_CHART
      );
    });
    ...
    it("return BAR_CHART when clinical attribute has data type as STRING", () => {
      ...
      assert.isTrue(
        getDefaultChartTypeByClinicalAttribute(attr) === ChartTypeEnum.BAR_CHART
      );
    });
  });
  ...
  describe("isOccupied", () => {
    ...
    it("Check the bigger chart starts from even index", () => {
      // x
      assert.isTrue(
        isOccupied([["1", "", "", "2", "", ""]], { x: 1, y: 0 }, { w: 2, h: 1 })
      );
      assert.isTrue(
    ...
    it("no filters selected and queriedSampleIdentifiers is empty", done => {
      getSamplesByExcludingFiltersOnChart(
        ...
        .then(() => {
          assert.isTrue(
            fetchStub.calledWith({
              studyViewFilter: { ...emptyStudyViewFilter, studyIds: ["study1"] }
            })
          );
          done();
        })
  ...
  describe("getFilteredSampleIdentifiers", () => {
    ...
    it("when filter function is present", () => {
      assert.deepEqual(
        getFilteredSampleIdentifiers(samples, sample => sample.sequenced),
        [{ sampleId: "sample1", studyId: "study1" }]
      );
      assert.deepEqual(
        getFilteredSampleIdentifiers(
  ...
  describe("getFilteredStudiesWithSamples", () => {
    const samples: Sample[] = [
      { sampleId: "sample1", studyId: "study1", uniqueSampleKey: "sample1" }
    ] as any;
