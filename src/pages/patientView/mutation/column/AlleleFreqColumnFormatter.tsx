...
export default class AlleleFreqColumnFormatter {
  ...
  static indexToBarLeft = (n: number) =>
    n *
    (AlleleFreqColumnFormatter.barWidth + AlleleFreqColumnFormatter.barSpacing);
  ...
  public static renderFunction(
    ...
    const freqs = sampleOrder.map(
      (sampleId: string) =>
        (sampleToElements[sampleId] && sampleToElements[sampleId].freq) ||
        undefined
    );
    ...
    if (sampleManager.samples.length === 1) {
      content = (
        <span>
          {!isNaN(freqs[0]) ? getFormattedFrequencyValue(freqs[0]) : ""}
        </span>
      );
    }
  ...
  public static getSortValue(
    ...
    return sampleManager
      ...
      .map(
        mutation => AlleleFreqColumnFormatter.calcFrequency(mutation) || null
      );
  }
