...
> {
      ...
      ([query, hash, pathname]) => {
        ...
        if (validationResult.isValid) {
          ...
          if (navCaseIdMatch && navCaseIdMatch.length > 1) {
            patientViewPageStore.patientIdsInCohort = parseCohortIds(
              navCaseIdMatch[1]
            );
          }
        } else {
  ...
  @computed get cnaTableStatus() {
    if (patientViewPageStore.molecularProfileIdDiscrete.isComplete) {
      if (
        patientViewPageStore.molecularProfileIdDiscrete.result === undefined
      ) {
        return "unavailable";
  ...
  hideTissueImageTab() {
    return (
      patientViewPageStore.hasTissueImageIFrameUrl.isPending ||
      patientViewPageStore.hasTissueImageIFrameUrl.isError ||
    ...
    invoke: async () => {
      if (!_.isEmpty(patientViewPageStore.getWholeSlideViewerIds.result)) {
        const url = getWholeSlideViewerUrl(
          patientViewPageStore.getWholeSlideViewerIds.result!,
  ...
  public render() {
    ...
    if (patientViewPageStore.urlValidationError) {
      return (
        <ValidationAlert
          urlValidationError={patientViewPageStore.urlValidationError}
        />
      );
    }
    ...
    if (
      ...
      patientViewPageStore.studyMetaData.isComplete
    ) {
          ...
          return (
            <div className="patientSample">
                  ...
                  <span style={{ display: "inline-flex" }}>
                    ...
                    <a
                      href={getSampleViewUrl(
                        patientViewPageStore.studyMetaData.result!.studyId,
                        sample.id
                      )}
                      target="_blank"
                      onClick={(e: React.MouseEvent<HTMLAnchorElement>) =>
                        this.handleSampleClick(sample.id, e)
                      }
                    >
              ...
              {patientViewPageStore.hasMutationalSignatureData.result ===
                ...
                <LoadingIndicator
                  isLoading={
                    patientViewPageStore.mutationalSignatureData.isPending &&
                    patientViewPageStore.mutationalSignatureMetaData.isPending
                  }
                />
              )}
              ...
              {patientViewPageStore.hasMutationalSignatureData.result ===
                ...
                patientViewPageStore.mutationalSignatureData.isComplete &&
                patientViewPageStore.mutationalSignatureMetaData.isComplete && (
                  <SignificantMutationalSignatures
                    data={patientViewPageStore.mutationalSignatureData.result}
                    metadata={
                      patientViewPageStore.mutationalSignatureMetaData.result
                    }
      ...
      if (
        ...
        patientViewPageStore.pageMode === "sample" &&
        patientViewPageStore.patientId &&
        patientViewPageStore.allSamplesForPatient &&
        patientViewPageStore.allSamplesForPatient.result.length > 1
      ) {
        sampleHeader.push(
          <button
            className="btn btn-default btn-xs"
            onClick={() =>
              this.handlePatientClick(patientViewPageStore.patientId)
            }
          >
            Show all {patientViewPageStore.allSamplesForPatient.result.length}{" "}
            samples
          </button>
        );
      }
    ...
    if (
      patientViewPageStore.patientIdsInCohort &&
      patientViewPageStore.patientIdsInCohort.length > 0
    ) {
      const indexInCohort = patientViewPageStore.patientIdsInCohort.indexOf(
        patientViewPageStore.studyId + ":" + patientViewPageStore.patientId
      );
      cohortNav = (
        <PaginationControls
          ...
          nextPageDisabled={
            indexInCohort === patientViewPageStore.patientIdsInCohort.length - 1
          }
          lastPageDisabled={
            indexInCohort === patientViewPageStore.patientIdsInCohort.length - 1
          }
          onFirstPageClick={() =>
            this.handlePatientClick(patientViewPageStore.patientIdsInCohort[0])
          }
          onPreviousPageClick={() =>
            this.handlePatientClick(
              patientViewPageStore.patientIdsInCohort[indexInCohort - 1]
            )
          }
          onNextPageClick={() =>
            this.handlePatientClick(
          ...
          onLastPageClick={() =>
            this.handlePatientClick(
              patientViewPageStore.patientIdsInCohort[
                patientViewPageStore.patientIdsInCohort.length - 1
              ]
            )
          }
          onChangeCurrentPage={newPage => {
            if (
              newPage > 0 &&
              newPage <= patientViewPageStore.patientIdsInCohort.length
            ) {
              this.handlePatientClick(
                patientViewPageStore.patientIdsInCohort[newPage - 1]
              );
            }
          }}
      ...
      <PageLayout noMargin={true} hideFooter={true}>
        {patientViewPageStore.patientViewData.isComplete && (
          <Helmet>
            ...
            <meta
              name="description"
              content={patientViewPageStore.metaDescription}
            />
          </Helmet>
        )}
                      ...
                      <td>
                        <PatientHeader
                          handlePatientClick={(id: string) =>
                            this.handlePatientClick(id)
                          }
                          patient={
                            patientViewPageStore.patientViewData.result.patient
                          }
          ...
          <If condition={patientViewPageStore.patientViewData.isComplete}>
            <Then>
              ...
              >
                <MSKTab key={0} id="summary" linkText="Summary">
                  ...
                  <LoadingIndicator
                    isLoading={
                      ...
                      patientViewPageStore.cnaSegments.isPending
                    }
                  />
                  ...
                  {patientViewPageStore.mutationData.isComplete &&
                    ...
                    (patientViewPageStore.mutationData.result.length > 0 ||
                      patientViewPageStore.cnaSegments.result.length > 0) && (
                      <div>
                        <GenomicOverview
                          mergedMutations={
                            patientViewPageStore.mergedMutationData
                          }
                  ...
                  <LoadingIndicator
                    isLoading={
                      ...
                      patientViewPageStore.studyIdToStudy.isPending
                    }
                  />
                  ...
                  {patientViewPageStore.oncoKbAnnotatedGenes.isComplete &&
                    ...
                    patientViewPageStore.studyIdToStudy.isComplete && (
                      <PatientViewMutationTable
                        studyIdToStudy={
                          patientViewPageStore.studyIdToStudy.result
                        }
                        sampleManager={sampleManager}
                        sampleIds={
                          sampleManager
                        ...
                        variantCountCache={
                          patientViewPageStore.variantCountCache
                        }
                        indexedVariantAnnotations={
                          patientViewPageStore.indexedVariantAnnotations
                        }
                        discreteCNACache={patientViewPageStore.discreteCNACache}
                        mrnaExprRankCache={
                          patientViewPageStore.mrnaExprRankCache
                        }
                        oncoKbEvidenceCache={
                          patientViewPageStore.oncoKbEvidenceCache
                        }
                        ...
                        genomeNexusMyVariantInfoCache={
                          patientViewPageStore.genomeNexusMyVariantInfoCache
                        }
                        mrnaExprRankMolecularProfileId={
                          patientViewPageStore.mrnaRankMolecularProfileId
                            .result || undefined
                        }
                        discreteCNAMolecularProfileId={
                          patientViewPageStore.molecularProfileIdDiscrete.result
                        }
                        data={
                          patientViewPageStore.mergedMutationDataIncludingUncalled
                        }
                        downloadDataFetcher={
                          patientViewPageStore.downloadDataFetcher
                        }
                        mutSigData={patientViewPageStore.mutSigData.result}
                        myCancerGenomeData={
                          patientViewPageStore.myCancerGenomeData
                        }
                        ...
                        oncoKbCancerGenes={
                          patientViewPageStore.oncoKbCancerGenes
                        }
                  ...
                  {patientViewPageStore.studyIdToStudy.isComplete &&
                    patientViewPageStore.referenceGenes.isComplete && (
                      <CopyNumberTableWrapper
                        ...
                        pubMedCache={patientViewPageStore.pubMedCache}
                        referenceGenes={
                          patientViewPageStore.referenceGenes.result
                        }
                        data={patientViewPageStore.mergedDiscreteCNAData}
                        copyNumberCountCache={
                          patientViewPageStore.copyNumberCountCache
                        }
                        mrnaExprRankCache={
                          patientViewPageStore.mrnaExprRankCache
                        }
                ...
                <MSKTab key={2} id="clinicalData" linkText="Clinical Data">
                  <div className="clearfix">
                    <FeatureTitle
                      title="Patient"
                      isLoading={
                        patientViewPageStore.clinicalDataPatient.isPending
                      }
                      className="pull-left"
                    />
                  ...
                  <div className="clearfix">
                    <FeatureTitle
                      title="Samples"
                      isLoading={
                        patientViewPageStore.clinicalDataGroupedBySample
                          .isPending
                      }
                      className="pull-left"
                    />
                ...
                >
                  <div>
                    <PathologyReport
                      ...
                      pdfs={patientViewPageStore.pathologyReport.result}
                    />
                  </div>
                </MSKTab>
                ...
                {patientViewPageStore.studyId === "mskimpact" &&
                  this.wholeSlideViewerUrl.result && (
                    ...
                    >
                      <div>
                        <IFrameLoader
                          height={WindowStore.size.height - 220}
                          url={this.wholeSlideViewerUrl.result!}
                        />
                      </div>
                    </MSKTab>
                  )}
                  ...
                  <MSKTab key={7} id="trialMatchTab" linkText="Matched Trials">
                    <TrialMatchTable
                      ...
                      detailedTrialMatches={
                        patientViewPageStore.detailedTrialMatches.result
                      }
                      containerWidth={WindowStore.size.width - 20}
                    />
                  </MSKTab>
