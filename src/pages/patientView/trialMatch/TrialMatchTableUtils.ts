...
export function groupTrialMatchesByAgeNumerical(
  ...
  const matches = _.map(matchesGroupedByAge, (ageGroup, age) => {
    ...
    const positiveAndNegativeMatches = groupTrialMatchesByGenomicAlteration(
...
export function groupTrialMatchesByGenomicAlteration(ageGroup: ITrialMatch[]) {
  const matchesGroupedByGenomicAlteration = _.groupBy(
    ageGroup,
    (trial: ITrialMatch) => trial.genomicAlteration
  );
  const matches: IGenomicGroupMatch[] = []; // positive matches
  const notMatches: IGenomicGroupMatch[] = []; // negative matches
  _.forEach(
    matchesGroupedByGenomicAlteration,
    (genomicAlterationGroup, genomicAlteration) => {
      const genomicGroupMatch = groupTrialMatchesByPatientGenomic(
        genomicAlterationGroup,
        genomicAlteration
      );
...
export function groupTrialMatchesByPatientGenomic(
  genomicAlterationGroup: ITrialMatch[],
  genomicAlteration: string
): IGenomicGroupMatch {
  const matchesGroupedByPatientGenomic = _.groupBy(
    genomicAlterationGroup,
    (trial: ITrialMatch) => trial.patientGenomic
  );
  const genomicGroupMatch: IGenomicGroupMatch = {
    genomicAlteration: genomicAlteration,
    matches: []
  };
  genomicGroupMatch.matches = _.map(
    ...
    (patientGenomicGroup: ITrialMatch[]) => {
      const genomicMatch: IGenomicMatch = {
        ...
        sampleIds: _.uniq(
          _.map(patientGenomicGroup, (trial: ITrialMatch) => trial.sampleId)
        )
      };
      return genomicMatch;
    }
