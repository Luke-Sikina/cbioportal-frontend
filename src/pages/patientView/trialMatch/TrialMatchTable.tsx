...
export default class TrialMatchTable extends React.Component<ITrialMatchProps> {
  ...
  get columnWidths() {
    return {
      ...
      [ColumnKey.MATCHING_CRITERIA]:
        0.65 * (this.props.containerWidth - ColumnWidth.ID)
    };
  }
                ...
                {armMatch.matches.map(
                  (
                    ...
                    <div className={styles.criteriaContainer}>
                      ...
                      <If condition={cgIndex < armMatch.matches.length - 1}>
    ...
    return (
      <div className={styles.firstRight}>
        <span className={styles.secondLeft}>
          {clinicalGroupMatch.trialAgeNumerical + " yrs old"}
        </span>
        <span className={styles.secondRight}>
          {clinicalGroupMatch.trialOncotreePrimaryDiagnosis.positive.join(", ")}
          {clinicalGroupMatch.trialOncotreePrimaryDiagnosis.negative.length >
            0 && (
            <span>
              <b> except in </b>
              <If
  ...
  public getGenomicMatch(clinicalGroupMatch: IClinicalGroupMatch) {
    return (
      <div className={styles.firstLeft}>
        <div>
          {clinicalGroupMatch.matches.map(
                ...
                <If
                  condition={
                    ...
                    genomicGroupMatch.genomicAlteration ===
                      `${genomicGroupMatch.matches[0].trueHugoSymbol} ${genomicGroupMatch.matches[0].trueProteinChange}`
                  }
                >
                  <Then>{this.getGenomicExactMatch(genomicGroupMatch)}</Then>
                  <Else>
        ...
        <div>
          {clinicalGroupMatch.notMatches.length > 0 && (
            ...
            >
              {this.mainContent(
                `No alterations in ${this.getHugoSymbolName(
                  clinicalGroupMatch.notMatches,
                  3
        ...
        <Then>
          <ul className={styles.alterationUl}>
            {genomicGroupMatch.matches.map((genomicMatch: IGenomicMatch) => (
              <li>
                {`${genomicMatch.trueHugoSymbol} ${genomicMatch.trueProteinChange} `}
      ...
      <div className={styles.tooltip}>
        {data.map((genomicGroupMatch: IGenomicGroupMatch) => (
          <div className={styles.genomicSpan}>
            <b>Not </b>
            {`${genomicGroupMatch.genomicAlteration.replace(/!/g, "")} `}
            {genomicGroupMatch.matches[0].sampleIds.map((sampleId: string) => (
              <span className={styles.genomicSpan}>
                {this.props.sampleManager!.getComponentForSample(
                  sampleId,
  ...
  public getHugoSymbolName(matches: IGenomicGroupMatch[], threshold: number) {
    const hugoSymbolSet = new Set(
      [...matches].map(x => x.genomicAlteration.split(" ")[0].slice(1))
    );
