    ...
    for (const component of components) {
      if (
        _.some(
          ...
          componentRect =>
            rectangleArea(rectangleIntersection(rect, componentRect)) > 0
        )
      ) {
...
export function layoutConnectedComponents(setRectangles: SetRectangles) {
  ...
  if (connectedComponents.length > 1) {
    ...
    const xPadding =
      Math.max(...boundingBoxes.map(box => box.xRange.max - box.xRange.min)) /
      10;
    const yPadding =
      Math.max(...boundingBoxes.map(box => box.yRange.max - box.yRange.min)) /
      10;
    ...
    switch (boundingBoxes.length) {
      ...
      case 3:
        targetCoordinates = [
          ...
          {
            x:
              Math.max(boundingBoxes[0].xLength, boundingBoxes[2].xLength) +
              xPadding,
            y: 0
          },
          {
            x: 0,
            y:
              Math.max(boundingBoxes[0].yLength, boundingBoxes[1].yLength) +
              yPadding
          }
        ];
        break;
