...
> {
  ...
  private async renameGroup(newName: string, group: StudyViewComparisonGroup) {
    ...
    await comparisonClient.updateGroup(group.uid, { name: newName, ...rest });
    this.props.store.notifyComparisonGroupsChange();
  }
  ...
  private readonly groupsSection = MakeMobxView({
    ...
    render: () => {
      if (this.props.store.comparisonGroups.result!.length > 0) {
        ...
        return (
          <div className={styles.noGroupsMessage}>
            Group comparison allows you to create custom groups and compare
            their clinical and genomic features. Use the button below to create
            groups based on selections.
          </div>
        );
      }
    },
  ...
  private get addGroupPanel() {
    ...
    if (this.addGroupPanelOpen) {
      contents = [
        ...
        >
          <h5>
            Add{selectedSamples ? ` ${selectedSamples.length}` : ""} selected
            samples
          </h5>
          <button
        ...
        <div style={{ width: "100%", marginTop: 7 }}>
          ...
          >
            <DefaultTooltip
              visible={
                allGroupNames && allGroupNames.includes(this.inputGroupName)
              }
              overlay={
                <div>
                  <i
      ...
      contents = (
        <button
          ...
          disabled={
            !selectedSamples || !this.props.store.entrezGeneIdToGene.isComplete
          }
          style={{ width: "100%" }}
        >
