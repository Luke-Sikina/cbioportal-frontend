...
export function getDefaultGroupName(
  ...
  const equalityFilters = _.sortBy(
    // sort clinical data equality filters into a canonical order - lets just do alphabetical by attribute id
  ...
  const mutatedGenes = _.flattenDeep<number>(
    (filters.mutatedGenes || []).map(filter => filter.entrezGeneIds)
  ).map(
    entrezGeneId => `${entrezGeneIdToGene[entrezGeneId].hugoGeneSymbol} mutant`
  );

  const cnaGenes = _.flattenDeep<CopyNumberGeneFilterElement>(
    (filters.cnaGenes || []).map(filter => filter.alterations)
  ).map(filterElt => {
    return `${entrezGeneIdToGene[filterElt.entrezGeneId].hugoGeneSymbol} ${
      filterElt.alteration === 2 ? "amp" : "del"
    }`;
  });
  ...
  return (
    <div style={{ width: 380 }}>
      <div style={{ marginBottom: 7 }}>
        The following samples cannot be found in our database. They might have
        been removed or changed since this group was created:{" "}
      </div>
...
export function getQuartiles<D extends { value: string }>(data: D[]) {
  data = _.chain(data)
  ...
  const quartileLimits = [
    0,
    Math.floor(quarterLength),
    Math.floor(2 * quarterLength),
    Math.floor(3 * quarterLength),
    data.length
  ];
  const quartiles: D[][] = [];
