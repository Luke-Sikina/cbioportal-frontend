...
> {
  ...
  @computed get showLogScaleControls() {
    return (
      this.highlightedRow!.clinicalAttribute.datatype.toLowerCase() === "number"
    );
  }
  ...
  private readonly groupSampleDataPromise = remoteData({
    await: () => [
      this.props.store.sampleKeyToActiveGroups,
    ...
    invoke: async () => {
      ...
      if (this.highlightedRow) {
        const axisData_Data = axisData.data;
        _.forEach(
          this.props.store.sampleKeyToActiveGroups.result!,
          (groupUids, uniqueSampleKey) => {
            axisData_Data.push({
              uniqueSampleKey,
          ...
          if (this.boxPlotData.isComplete) {
            plotElt = (
              <PlotsTabBoxPlot
                ...
                stroke={
                  mutationSummaryToAppearance[MutationSummary.Neither].stroke
                }
                strokeOpacity={
