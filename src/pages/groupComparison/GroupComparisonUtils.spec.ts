...
describe("GroupComparisonUtils", () => {
      ...
      assert.deepEqual(
        ...
        [
          makeGroup({
            name: "group1",
            studies: [
              { id: "study1", samples: ["sample2"], patients: ["patient1"] }
            ],
            color: "color1",
            uid: "uid1",
            nonExistentSamples: [
      ...
      assert.deepEqual(
        getOverlapFilteredGroups(
          [
            ...
            makeGroup({
              name: "group2",
              studies: [
                { id: "study1", samples: ["sample1", "sample2"], patients: [] },
                {
                  id: "study2",
                  samples: ["sample1", "sample2"],
                  patients: ["patient1"]
        ...
        [
          ...
          makeGroup({
            name: "group2",
            studies: [
              { id: "study2", samples: ["sample1"], patients: ["patient1"] }
            ],
            color: "color2",
            uid: "uid2",
            nonExistentSamples: [
      ...
      assert.deepEqual(
        getNumPatients({
          studies: [
            ...
            { id: "study3", samples: ["sample1"], patients: ["patient2"] }
          ]
        }),
        4
      );
  ...
  describe("getQuartiles", () => {
    it("gets quartiles of 1,2,3, and 4 values", () => {
      ...
      assert.deepEqual(getQuartiles(vals.slice(0, 1)), [[vals[0]]]);
      assert.deepEqual(getQuartiles(vals.slice(0, 2)), [[vals[0]], [vals[1]]]);
      assert.deepEqual(getQuartiles(vals.slice(0, 3)), [
        [vals[0]],
        [vals[1]],
        [vals[2]]
      ]);
      assert.deepEqual(getQuartiles(vals), [
        [vals[0]],
        [vals[1]],
        [vals[2]],
        [vals[3]]
    ...
    it("gets quartiles of 12,13,14,15 values", () => {
      ...
      assert.deepEqual(getQuartiles(vals.slice(0, 12)), [
        vals.slice(0, 3),
        vals.slice(3, 6),
        vals.slice(6, 9),
        vals.slice(9, 12)
      ]);
      assert.deepEqual(getQuartiles(vals.slice(0, 13)), [
        vals.slice(0, 3),
        vals.slice(3, 6),
        vals.slice(6, 9),
        vals.slice(9, 13)
      ]);
      assert.deepEqual(getQuartiles(vals.slice(0, 14)), [
        vals.slice(0, 3),
        vals.slice(3, 7),
        vals.slice(7, 10),
        vals.slice(10, 14)
      ]);
      assert.deepEqual(getQuartiles(vals.slice(0, 15)), [
        vals.slice(0, 3),
        vals.slice(3, 7),
        vals.slice(7, 11),
        vals.slice(11, 15)
