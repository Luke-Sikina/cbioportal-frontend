...
export default class PlotsTab extends React.Component<IPlotsTabProps, {}> {
  ...
  @computed get horzDatatypeOptions() {
    ...
    if (
      ...
      this.vertSelection.dataType === AlterationTypeConstants.GENERIC_ASSAY
    ) {
      noneDatatypeOption = [
        {
  ...
  @computed get vertDatatypeOptions() {
    ...
    if (
      this.dataTypeOptions &&
      this.horzSelection.dataType === AlterationTypeConstants.GENERIC_ASSAY
    ) {
      noneDatatypeOption = [
        {
          value: NONE_SELECTED_OPTION_STRING_VALUE,
  ...
  @computed get vertGenesetOptions() {
    ...
    return (sameGenesetOption || []).concat((this.horzGenesetOptions.result ||
      []) as any[]);
  }
  ...
  @computed get vertTreatmentOptions() {
    ...
    if (this.horzTreatmentOptions || this.horzSelection.dataType) {
      if (
        ...
        if (
          this.horzSelection.dataType &&
          this.showTreatmentSelectBox(this.horzSelection.dataType) &&
          this.horzSelection.selectedTreatmentOption &&
          this.horzSelection.selectedTreatmentOption.value !==
            NONE_SELECTED_OPTION_STRING_VALUE
        ) {
          sameTreatmentOption = [
            {
              value: SAME_SELECTED_OPTION_STRING_VALUE,
              label: `Same treatment (${this.horzSelection.selectedTreatmentOption.label})`
            }
          ];
        }
      }
  ...
  readonly clinicalAttributeIdToClinicalAttribute = remoteData<{
    ...
    invoke: () => {
      let _map: { [clinicalAttributeId: string]: ClinicalAttribute } = _.keyBy(
  ...
  readonly dataTypeOptions = remoteData<{ value: string; label: string }[]>({
    ...
    invoke: () => {
      ...
      if (
        ...
        dataTypeIds.includes("GENESET_SCORE")
      ) {
  ...
  @computed get waterfallPlotTitle(): string {
    const treatment =
      this.horzSelection.treatmentId || this.vertSelection.treatmentId;
    const gene = this.utilitiesMenuSelection.selectedGeneOption!.label;
    const profileName = this.horzSelection.selectedDataSourceOption
      ? this.horzSelection.selectedDataSourceOption!.label
      : this.vertSelection.selectedDataSourceOption!.label;
    let geneStyle = "";
    if (this.viewMutationType) {
  ...
  @computed get waterfallPlotWidth(): number {
    ...
    return (
      WATERFALLPLOT_BASE_SIDELENGTH +
      Math.round(
        noSamples * WATERFALLPLOT_SIDELENGTH_SAMPLE_MULTIPLICATION_FACTOR
      )
    );
  }
  ...
  @computed get boxPlotTooltip() {
    return (d: IBoxScatterPlotPoint) => {
      ...
      if (this.boxPlotData.isComplete) {
        ...
        content = (
          <span>
            Loading... (this shouldnt appear because the box plot shouldnt be
            visible)
          </span>
        );
      }
  ...
  @computed get scatterPlotHighlight() {
    ...
    return (d: IPlotSampleData) => {
      ...
      for (const word of searchCaseWords) {
        caseMatch =
          caseMatch || (!!word.length && d.sampleId.indexOf(word) > -1);
        if (caseMatch) {
          break;
        }
      }
  ...
  private fDatumHasMutation = (d: any, mutation: string) => {
    const mutationFound =
      ...
      !!d.mutations.find(
        (m: any) =>
          !!(m.proteinChange && m.proteinChange.indexOf(mutation) > -1)
      );
  ...
  private getAxisMenu(
    ...
    if (
      ...
      if (axisSelection.dataType === CLIN_ATTR_DATA_TYPE) {
        dataSourceDescription = this.clinicalAttributeIdToClinicalAttribute
          .result![dataSourceValue].description;
      } else {
        dataSourceDescription = this.props.store
          .molecularProfileIdToMolecularProfile.result![dataSourceValue]
          .description;
      }
    }
    ...
    if (axisSelection.dataType === NONE_SELECTED_OPTION_STRING_VALUE) {
      ...
      const otherProfileName = this.props.store
        .molecularProfileIdToMolecularProfile.result![otherDataSourceId!].name;
  ...
  readonly waterfallPlotData = remoteData<{ data: IWaterfallPlotData[] }>({
    ...
    invoke: () => {
      ...
      if (!horzAxisData && !vertAxisData) {
        return new Promise<{ horizontal: boolean; data: IWaterfallPlotData[] }>(
          () => 0
        ); // dont resolve
      } else {
        ...
        const ensemblGeneId = this.utilitiesMenuSelection.selectedGeneOption!
          .value;
        const selectedGene = this.props.store.entrezGeneIdToGene.result![
          ensemblGeneId
        ];
  ...
  @computed get waterfallPlotSortOrder(): string {
    if (this._waterfallPlotSortOrder === undefined && this.isWaterfallPlot) {
      const dataSourceId =
        this.horzSelection.dataSourceId || this.vertSelection.dataSourceId;
  ...
  @computed get isWaterfallPlot(): boolean {
    return (
      !!this.plotType.result && this.plotType.result === PlotType.WaterfallPlot
    );
  }
    ...
    if (this.showNoTreamentsSelectedWarning) {
      return (
        <div>
          ...
          &nbsp;
          <span>
            To visualize treatment response data, you must first visit the
            OncoPrint tab and use the "Heatmap" menu to add treatment response
            tracks to the OncoPrint. Any treatments added to the OncoPrint will
            then be available on this tab for visualization.
          </span>
        </div>
      );
    }
          ...
          <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        ...
                        className={`gene-select-background ${geneSelectShownClassName}`}
                      >
                        {this.isWaterfallPlot && (
            ...
            {this.mutationDataCanBeShown && (
              <div style={{ marginTop: 5 }}>
                * Driver annotation settings are located in the Mutation Color
            ...
            {this.limitValuesCanBeShown &&
              this.plotType.result === PlotType.BoxPlot && (
                <div style={{ marginTop: 5 }}>
                  <div>
                    ** Labeling of threshold values (e.g. >8.00) excludes
                    threshold values from box plot calculation.
                  </div>
                </div>
              )}
