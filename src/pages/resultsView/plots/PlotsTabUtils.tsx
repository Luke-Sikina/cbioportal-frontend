...
function scatterPlotMutationLegendData(
  ...
  const legendData: any[] = _.chain(mutationLegendOrder)
    ...
    .map(type => {
      ...
      const legendSymbol =
        plotType === PlotType.WaterfallPlot ? "square" : appearance.symbol;
      const stroke =
        plotType === PlotType.WaterfallPlot
          ? appearance.fill
          : appearance.stroke;
          ...
          proteinChangeComponents.push(
            <span key={mutation.proteinChange}>
              ...
              <span style={{ marginLeft: 1 }}>
                ...
                {mutation.oncoKbOncogenic ? oncoKbIcon(mutation) : null}
              </span>
            </span>
          );
        ...
        for (const alt of entry[1]) {
          if (alt.value in cnaToAppearance) {
            alterationComponents.push(
              <span key={alt.value}>
                {(cnaToAppearance as any)[alt.value].legendLabel}
                <span style={{ marginLeft: 1 }}>
                  {alt.oncoKbOncogenic ? oncoKbIcon(alt) : null}
                </span>
              </span>
            );
...
function generalScatterPlotTooltip<D extends IPlotSampleData>(
  ...
  return (
    <div>
      ...
      <div>
        Horizontal: <span style={{ fontWeight: "bold" }}> {horzLabel}</span>
      </div>
      <div>
        Vertical: <span style={{ fontWeight: "bold" }}> {vertLabel}</span>
      </div>
      {mutationsSection}
      {!!mutationsSection && <br />}
      {cnaSection}
...
export function boxPlotTooltip(d: IBoxScatterPlotPoint, horizontal: boolean) {
  ...
  let horzThresholdTypeKey = horizontal ? "thresholdType" : (undefined as any);
  let vertThresholdTypeKey = horizontal ? undefined : ("thresholdType" as any);
  return generalScatterPlotTooltip(
    d,
    horzAxisKey,
    vertAxisKey,
...
export function makeScatterPlotData(
  ...
  for (const d of horzData.data) {
    ...
    if (mutations || copyNumberAlterations) {
      ...
      if (mutations) {
        ...
        for (const molecularProfileId of mutations.molecularProfileIds) {
          profiledMutations =
            profiledMutations || !!profiledReport[molecularProfileId];
        }
      }
    ...
    if (profiledMutations) {
      const genesMutatedCount = _.uniqBy(sampleMutations, m => m.entrezGeneId)
        .length;
  ...
  for (const d of vertData.data) {
    ...
    if (datum) {
      ...
      datum.vertThresholdTypes = ([] as string[]).concat(d.thresholdType || "");
  ...
  for (const d of axisData.data) {
    ...
    if (sampleMutations && sampleMutations.length) {
      const counts = _(sampleMutations)
        .filter(
          (d: AnnotatedMutation) => d.entrezGeneId === selectedGene.entrezGeneId
...
export function showWaterfallPlot(
  ...
  return (
    (vertSelection.dataType === AlterationTypeConstants.GENERIC_ASSAY &&
...
export function getScatterPlotDownloadData(
  ...
  for (const datum of data) {
    ...
    if (datum.mutations.length) {
      row.push(
        mutationsProteinChanges(datum.mutations, entrezGeneIdToGene).join("; ")
      );
      hasMutations = true;
    ...
    for (const d of vertAxisData.data) {
      if (Array.isArray(d.value) && d.value.length > 1) {
        ...
        sampleToNumPoints[d.uniqueSampleKey] =
          (sampleToNumPoints[d.uniqueSampleKey] || 1) * d.value.length;
      }
    }
  }
...
export function makeClinicalAttributeOptions(
...
) {
  {
    ...
    // sort
    let options = _.sortBy<{ value: string; label: string; priority: number }>(
