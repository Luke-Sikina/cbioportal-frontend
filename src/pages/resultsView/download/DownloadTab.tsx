...
> {
  ...
  readonly mutationDataText = remoteData<string>({
    await: () => [this.mutationDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.mutationDownloadData.result!))
  });
  ...
  readonly transposedMutationDataText = remoteData<string>({
    ...
    invoke: () =>
      Promise.resolve(
        stringify2DArray(this.transposedMutationDownloadData.result!)
      )
  });
  ...
  readonly mrnaDataText = remoteData<string>({
    await: () => [this.mrnaDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.mrnaDownloadData.result!))
  });

  readonly transposedMrnaDataText = remoteData<string>({
    await: () => [this.transposedMrnaDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.transposedMrnaDownloadData.result!))
  });
  ...
  readonly proteinDataText = remoteData<string>({
    await: () => [this.proteinDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.proteinDownloadData.result!))
  });
  ...
  readonly transposedProteinDataText = remoteData<string>({
    ...
    invoke: () =>
      Promise.resolve(
        stringify2DArray(this.transposedProteinDownloadData.result!)
      )
  });
  ...
  readonly cnaDataText = remoteData<string>({
    await: () => [this.cnaDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.cnaDownloadData.result!))
  });

  readonly transposedCnaDataText = remoteData<string>({
    await: () => [this.transposedCnaDownloadData],
    invoke: () =>
      Promise.resolve(stringify2DArray(this.transposedCnaDownloadData.result!))
  });
      ...
      this.props.store.oqlFilteredCaseAggregatedDataByUnflattenedOQLLine.result!.forEach(
        (data, index) => {
          ...
          // or data is for merged track (group: list of oqls)
            ...
            labels.push(
              getMultipleGeneResultKey(data.oql as MergedTrackLineFilterOutput<
                AnnotatedExtendedAlteration
              >)
            );
          }
          ...
          if (data.mergedTrackOqlList === undefined) {
            ...
            if (singleTrackOql.parsed_oql_line.alterations) {
              trackAlterationTypesMap[label] = _.uniq(
                _.map(singleTrackOql.parsed_oql_line.alterations, alteration =>
              ...
              (oql: OQLLineFilterOutput<AnnotatedExtendedAlteration>) => {
                if (oql.parsed_oql_line.alterations) {
                  const types: string[] = _.map(
                    ...
                    alteration => alteration.alteration_type.toUpperCase()
                  );
        ...
        return (
          <WindowWidthBox data-test="downloadTabDiv" offset={60}>
            ...
            <div>
              ...
              >
                <tbody>
                  ...
                  {hasValidMutationData(this.mutationData.result!) &&
                  ...
                  {this.alteredSamplesDownloadControls(
                    ...
                    this.props.store.virtualStudyParams.result!
                  )}
                  {this.unalteredSamplesDownloadControls(
                    this.unalteredCaseAlterationData.result!,
                    this.props.store.virtualStudyParams.result!
                  )}
            ...
            >
              ...
              />
              <GeneAlterationTable
                geneAlterationData={this.geneAlterationData.result!}
              />
            </div>
            <hr />
  ...
  private alteredSamplesDownloadControls(
    ...
    const alteredSampleCaseIds = _.map(
      ...
      caseAlteration => `${caseAlteration.studyId}:${caseAlteration.sampleId}`
    );
    ...
    const alteredSamplesVirtualStudyParams = {
      ...
      selectedSamples: _.filter(
        ...
        (sample: Sample) =>
          alteredSampleCaseIds.includes(`${sample.studyId}:${sample.sampleId}`)
      ),
  ...
  private handleMutationDownload() {
    onMobxPromise(this.mutationDataText, text =>
      fileDownload(text, "mutations.txt")
    );
  }

  private handleTransposedMutationDownload() {
    onMobxPromise(this.transposedMutationDataText, text =>
      fileDownload(text, "mutations_transposed.txt")
    );
  }

  private handleMrnaDownload() {
    onMobxPromise(this.mrnaDataText, text =>
      fileDownload(text, "mRNA_exp.txt")
    );
  }

  private handleTransposedMrnaDownload() {
    onMobxPromise(this.transposedMrnaDataText, text =>
      fileDownload(text, "mRNA_exp_transposed.txt")
    );
  }

  private handleProteinDownload() {
    onMobxPromise(this.proteinDataText, text =>
      fileDownload(text, "protein_exp.txt")
    );
  }

  private handleTransposedProteinDownload() {
    onMobxPromise(this.transposedProteinDataText, text =>
      fileDownload(text, "protein_exp_transposed.txt")
    );
  }
  ...
  private handleTransposedCnaDownload() {
    onMobxPromise(this.transposedCnaDataText, text =>
  ...
  private async handleVirtualStudyButtonClick(alterations: ICaseAlteration[]) {
    let studies = _.reduce(
      _.groupBy(alterations, "studyId"),
      (acc: { id: string; samples: string[] }[], alteration, studyId) => {
        acc.push({
          id: studyId,
    ...
    const saveStudyResult = await sessionServiceClient.saveVirtualStudy(
      parameters,
      false
    );
