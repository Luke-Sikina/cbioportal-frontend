  ...
  async getUrl(): Promise<ShareUrls> {
    ...
    if (this.props.sessionEnabled) {
      if (this.props.routingStore._session) {
        ...
        const resp = await this.props.routingStore.saveRemoteSession(
          this.props.routingStore.query
        );
        // for testing purposes we don't have links to localhost
        sessionUrl = URL.format({
          hostname: win.location.hostname.includes("localhost")
            ? "www.cbioportal.org"
    ...
    this.getUrl().then((urlData: ShareUrls) => {
      win.open(
        `https://twitter.com/intent/tweet?url=${encodeURIComponent(
          this.shortenedUrl(urlData)
        )}&text=${encodeURIComponent(document.title)}&via=cbioportal`
      );
    });
  }
