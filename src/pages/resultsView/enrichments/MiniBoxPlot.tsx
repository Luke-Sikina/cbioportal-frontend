...
@observer
export default class MiniBoxPlot extends React.Component<
  IMiniBoxPlotProps,
  {}
> {
  ...
  public render() {
    ...
    if (this.props.selectedGeneHugo) {
      molecularData = this.props.store.numericGeneMolecularDataCache.get({
        entrezGeneId: this.props.selectedGeneEntrez,
    ...
    if (
      ...
      this.scatterData = getBoxPlotScatterData(
        molecularData,
        molecularProfileId,
        this.props.store.oqlFilteredCaseAggregatedData.result!.samples,
        ...
        {this.props.selectedGeneHugo && this.scatterData.length > 0 && (
          <div>
            ...
            <div className="posRelative">
              <div
                className="borderedChart inlineBlock posRelative"
                data-test="MiniBoxPlot"
              >
                <DownloadControls
                ...
                >
                  <VictoryAxis
                    tickValues={[1, 2]}
                    tickFormat={(t: any) => (t === 1 ? "Altered" : "Unaltered")}
                    label={
                      "Query: " +
                      shortenGenesLabel(this.props.queryGenes, 2) +
                      "\n(q-Value: " +
                      toConditionalPrecision(
                        this.props.selectedGeneQValue,
                        3,
                        0.01
                      ) +
                      ")"
                  ...
                  />
                  <VictoryBoxPlot
                    data={getBoxPlotModels(this.scatterData)}
                    boxWidth={60}
                    whiskerWidth={30}
                    style={{
                  ...
                  <VictoryScatter
                    ...
                    events={events}
                    size={(datum: any, active: any) => (active ? 10 : 3)}
                    style={{
                      data: {
                        fill: "#58ACFA",
                        strokeWidth: 0,
              ...
              {this.tooltipModel && (
                <Popover
                  positionLeft={this.tooltipModel.x + 16}
                  positionTop={this.tooltipModel.y - 26}
                ...
                >
                  <a
                    href={getSampleViewUrl(
                      ...
                      this.tooltipModel.datum.sampleId
                    )}
                    target="_blank"
                  >
                    <b>{this.tooltipModel.datum.sampleId}</b>
                  </a>
                  <br />
                  mRNA expression: {this.tooltipModel.datum.y.toFixed(3)}
                  <br />
