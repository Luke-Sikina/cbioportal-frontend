        ...
        genes = _.flatMap(this.selectedGenes, geneWithAlteration => {
          ...
          _.each(geneWithAlteration.alterations, alteration => {
            if (
              ...
              geneKeys.push(
                geneWithAlteration.gene + ": " + alteration.constr_val
              );
            }
          });
          return geneKeys;
    ...
    let groupRows = _.map(this.props.groupOrder, groupName => {
      ...
      return (
        <tr style={style}>
          <td>{group.name}</td>
          <td>
            {group.alteredPercentage.toFixed(2)}% ({group.alteredCount}/
            {group.profiledCount})
          </td>
        </tr>
      );
...
class GenesSelection extends React.Component<IGeneSelectionProps, {}> {
  ...
  @computed get isCustomGeneSelection() {
    return (
      ...
      this.selectedGeneListOption.label === GeneOptionLabel.USER_DEFINED_OPTION
    );
  }
  ...
  @computed private get inSyncMode() {
    return (
      this._selectedGeneListOption &&
      this._selectedGeneListOption.label === GeneOptionLabel.SYNC_WITH_TABLE
    );
  }
        ...
        {!this.inSyncMode && !this.isCustomGeneSelection && (
          <div>
            ...
            <div style={{ display: "table-row" }}>
              <label style={{ display: "table-cell", whiteSpace: "nowrap" }}>
                Number of Genes (max. {this.props.maxNumberOfGenes}): &nbsp;
              </label>
              <FormControl
