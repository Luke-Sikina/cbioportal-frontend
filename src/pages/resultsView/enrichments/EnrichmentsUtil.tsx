...
export function getBoxPlotModels(scatterData: any[]): BoxPlotModel[] {
  const alteredBoxPlotData = calculateBoxPlotModel(
    scatterData.filter(s => s.x < 1.5).map(s => s.y)
  );
  ...
  const unalteredBoxPlotData = calculateBoxPlotModel(
    scatterData.filter(s => s.x > 1.5).map(s => s.y)
  );
...
export function getBoxPlotScatterData(
  molecularData: NumericGeneMolecularData[],
  molecularProfileId: string,
  sampleAlterations: any,
  alteredSampleKeys: string[]
): any[] {
  const scatterData: any[] = [];
  for (let uniqueSampleKey in sampleAlterations) {
    const alterations = sampleAlterations[uniqueSampleKey];
    const data: NumericGeneMolecularData | undefined = molecularData.find(
      m => m.uniqueSampleKey === uniqueSampleKey
    );
    if (data) {
      const y = molecularProfileId.includes("rna_seq")
        ? Math.log(data.value + 1) / Math.log(2)
        : data.value;
      const random = (seedrandom(y.toString())() - 0.5) * 0.5;
      scatterData.push({
        x: alteredSampleKeys.includes(uniqueSampleKey)
          ? 1 + random
          : 2 + random,
        y: y,
...
export function pickMRNAEnrichmentProfiles(profiles: MolecularProfile[]) {
  const mrnaProfiles = profiles.filter(p => {
    return (
      p.molecularAlterationType === AlterationTypeConstants.MRNA_EXPRESSION
    );
  });
...
export function getExpressionEnrichmentColumns(
  ...
  if (groups.length === 2) {
    ...
    columns.push({
      ...
      tooltip: (
        <span>
          Log2 of ratio of (unlogged) mean in {group1.name} to (unlogged) mean
          in {group2.name}
        </span>
      ),
  ...
  groups.forEach(group => {
    columns.push({
      ...
      tooltip: (
        <span>
          Mean log2 expression of the listed gene in {group.description}
        </span>
      ),
  ...
  if (includeAlteration) {
    dataWithOptionName = _.map(dataWithOptionName, datum => {
      return {
        ...datum,
        optionName:
          datum.hugoGeneSymbol + `: ${CNA_TO_ALTERATION[datum.value!]}`
      };
    });
  }

  let dataSortedByAlteredPercentage = _.clone(dataWithOptionName).sort(function(
    kv1,
    kv2
  ) {
    const t1 = _.reduce(
