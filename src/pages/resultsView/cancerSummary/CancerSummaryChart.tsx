...
> {
  ...
  private get chartWidth() {
    return (
      this.props.xLabels.length * (this.barWidth() + this.barSeparation()) + 100
    );
  }

  private get svgHeight() {
    return (
      this.barChartHeight() + this.scatterChartHeight + this.bottomPadding - 100
    );
  }
  ...
  @computed get scatterChartHeight() {
    ...
    const profiledDataTypesLength = _.uniqBy(this.scatterData, datum => datum.y)
      .length;
    if (profiledDataTypesLength > 0) {
      return (
        profiledDataTypesLength * (this.barWidth() + this.barSeparation()) +
        miscPadding
      );
    } else {
      return miscPadding;
