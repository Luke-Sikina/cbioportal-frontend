...
export class ResultsViewQuery {
  ...
  @observable.ref
  public samplesSpecification: SamplesSpecificationElement[] = [];
  ...
  @computed get hash(): number {
    ...
    const stringified = hashKeys.reduce(
  ...
  @computed public get rppaScoreThreshold() {
    return this._rppaScoreThreshold === undefined
      ? 2
      : this._rppaScoreThreshold;
  }
...
export function updateResultsViewQuery(
  ...
  if (
    !rvQuery.samplesSpecification ||
    !_.isEqual(rvQuery.samplesSpecification.slice(), samplesSpecification)
  ) {
  ...
  if (
    urlQuery.data_priority !== undefined &&
    parseInt(urlQuery.data_priority, 10) !== rvQuery.profileFilter
  ) {
  ...
  if (
    ...
    !_.isEqual(rvQuery.selectedMolecularProfileIds.slice(), profiles)
  ) {
  ...
  if (urlQuery.geneset_list) {
    ...
    const parsedGeneSetList = urlQuery.geneset_list.trim().length
      ? urlQuery.geneset_list.trim().split(/\s+/)
      : [];
  ...
  if (urlQuery.treatment_list) {
    // we have to trim because for some reason we get a single space from submission
    const parsedTreatmentList = urlQuery.treatment_list.trim().length
      ? urlQuery.treatment_list.trim().split(/;/)
      : [];
  ...
  if (
    !rvQuery.cohortIdsList ||
    !_.isEqual(_.sortBy(rvQuery.cohortIdsList), _.sortBy(cancerStudyIds))
  ) {
