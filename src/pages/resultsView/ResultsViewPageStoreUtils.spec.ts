...
describe("ResultsViewPageStoreUtils", () => {
  describe("computeCustomDriverAnnotationReport", () => {
    ...
    it("returns the right report for just tiers annotations, one element", () => {
      assert.deepEqual(
        computeCustomDriverAnnotationReport([driverTiersFilterMutation]),
        ...
        list: [
          {
            ...
            parsed_oql_line: {
              gene: "GENE1000",
              alterations: [
                { alteration_type: "exp", constr_rel: ">", constr_val: 2.5 }
              ]
            }
          },
          {
    ...
    it("annotates a few mutations", () => {
      assert.deepEqual(
        filterAndAnnotateMutations(
          [
            { mutationType: "missense", entrezGeneId: 1 } as Mutation,
            { mutationType: "in_frame_del", entrezGeneId: 1 } as Mutation,
  ...
  describe("getOncoKbOncogenic", () => {
    ...
    it("should return Oncogenic if thats the input", () => {
      assert.equal(
        getOncoKbOncogenic({ oncogenic: "Oncogenic" } as IndicatorQueryResp),
        "Oncogenic"
      );
    });
    ...
    it("should return empty string for any other case", () => {
      ...
      assert.equal(
        getOncoKbOncogenic({ oncogenic: "Inconclusive" } as IndicatorQueryResp),
        ""
      );
      assert.equal(
        getOncoKbOncogenic({ oncogenic: "Unknown" } as IndicatorQueryResp),
        ""
      );
      assert.equal(
      ...
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "Likely Oncogenic" } as IndicatorQueryResp),
          {
            profile: {
    ...
    it("annotates single element correctly in case of Predicted Oncogenic", () => {
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "Predicted Oncogenic" } as IndicatorQueryResp),
          {
            profile: {
              molecularAlterationType: "COPY_NUMBER_ALTERATION"
            } as MolecularProfile
    ...
    it("annotates single element correctly in case of Likely Neutral, Inconclusive, Unknown, asdfasd, undefined, empty", () => {
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "Likely Neutral" } as IndicatorQueryResp),
          {
            profile: {
              molecularAlterationType: "COPY_NUMBER_ALTERATION"
            } as MolecularProfile
      ...
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "Inconclusive" } as IndicatorQueryResp),
          {
            profile: {
              molecularAlterationType: "COPY_NUMBER_ALTERATION"
            } as MolecularProfile
      ...
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "Unknown" } as IndicatorQueryResp),
          {
            profile: {
              molecularAlterationType: "COPY_NUMBER_ALTERATION"
            } as MolecularProfile
      ...
      assert.deepEqual(
        annotateMolecularDatum(
          ...
          (d: NumericGeneMolecularData) =>
            ({ oncogenic: "asdfasdf" } as IndicatorQueryResp),
          {
            profile: {
              molecularAlterationType: "COPY_NUMBER_ALTERATION"
            } as MolecularProfile
  ...
  describe("getQueriedStudies", () => {
    ...
    it("when there only a subset of studies in studySampleMap compared to queriedIds", async () => {
      let test = await fetchQueriedStudies(
        { physical_study_1: { studyId: "physical_study_1" } as CancerStudy },
        ["physical_study_1", "physical_study_2"],
        virtualStudies
      );
      assert.deepEqual(_.map(test, obj => obj.studyId), [
...
describe("getSampleAlteredMap", () => {
  ...
  const studyToMolecularProfiles = {
    chol_nus_2012: [
      { molecularProfileId: "chol_nus_2012_mutations" } as MolecularProfile,
