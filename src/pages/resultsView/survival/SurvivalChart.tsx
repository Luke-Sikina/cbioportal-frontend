  ...
  @computed get scattersAndLines() {
    ...
    const scatterWithOpacityElements = analysisGroupsWithData.map(grp => (
      <VictoryScatter
        ...
        symbol="plus"
        style={{ data: { fill: grp.color, opacity: (d: any) => d.opacity } }}
        size={3}
      />
    ));
    const scatterElements = analysisGroupsWithData.map(grp => (
      <VictoryScatter
        ...
        symbol="circle"
        style={{
          data: { fill: grp.color, fillOpacity: this.hoverCircleFillOpacity }
        }}
        size={10}
        events={this.events}
      />
        ...
        {this.analysisGroupsWithData.map(group => (
          <span
            ...
            style={{
              ...
              marginRight: 12,
              fontWeight:
                this.highlightedCurve === group.value ? "bold" : "initial",
              cursor: "pointer"
            }}
            onClick={() => {
