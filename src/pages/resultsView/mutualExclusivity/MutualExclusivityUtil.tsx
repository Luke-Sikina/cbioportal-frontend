...
export function getCountsText(data: MutualExclusivity[]): JSX.Element {
  ...
  return (
    <p>
      The query contains {mutuallyExclusiveCounts[0]} with mutually exclusive
      alterations{mutuallyExclusiveCounts[1]}, and {coOccurentCounts[0]} with
      co-occurrent alterations{coOccurentCounts[1]}.
    </p>
  );
}
...
export function getSampleAlteredFilteredMap(
  ...
  _.forIn(isSampleAlteredMap, (alteredStatus, trackOql) => {
    if (alteredStatus && alteredStatus.length > 0) {
      if (
        alteredStatus.filter(status => status != AlteredStatus.UNPROFILED)
          .length > 0
      ) {
