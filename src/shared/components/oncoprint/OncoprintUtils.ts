...
import {
  OncoprintPatientGeneticTrackData,
  OncoprintSampleGeneticTrackData
...
function getAlterationInfoSequenced(
  ...
  const sequenced = sampleMode
    ? _.uniq(
        _.flatMap(geneSymbolArray, symbol => sequencedSampleKeysByGene[symbol])
      ).length
    : _.uniq(
        _.flatMap(geneSymbolArray, symbol => sequencedPatientKeysByGene[symbol])
      ).length;
  ...
  return remoteData<ClinicalTrackSpec[]>({
    ...
    invoke: async () => {
      ...
      return attributes.map((attribute: ClinicalAttribute) => {
        const data = oncoprint.props.store.clinicalDataCache.get(attribute)
          .result!;
  ...
  return remoteData<IHeatmapTrackSpec[]>({
    ...
    invoke: async () => {
      ...
      const geneProfiles = _.filter(
        molecularProfileIdToHeatmapTracks.values(),
        ...
        return {
          ...
          molecularAlterationType:
            molecularProfileIdToMolecularProfile[molecularProfileId]
              .molecularAlterationType,
          datatype:
            molecularProfileIdToMolecularProfile[molecularProfileId].datatype,
          data: makeHeatmapTrackData<
            IGeneHeatmapTrackDatum,
            "hugo_gene_symbol"
          >("hugo_gene_symbol", gene, sampleMode ? samples : patients, data),
          trackGroupIndex: molecularProfileIdToHeatmapTracks.get(
            molecularProfileId
          )!.trackGroupIndex,
          onRemove: action(() => {
            const trackGroup = molecularProfileIdToHeatmapTracks.get(
              molecularProfileId
            );
            if (trackGroup) {
            ...
            if (
              !molecularProfileIdToHeatmapTracks.has(molecularProfileId) &&
...
export function makeTreatmentProfileHeatmapTracksMobxPromise(
  ...
  return remoteData<IHeatmapTrackSpec[]>({
    ...
    invoke: async () => {
      ...
      const neededTreatments = _.flatten(
        treatmentProfiles.map(v => v.entities.keys())
      );
      await oncoprint.props.store.treatmentCache.getPromise(
        neededTreatments.map(g => ({ treatmentId: g })),
        true
      );
      ...
      const cacheQueries = _.flatten(
        treatmentProfiles.map(entry =>
          entry.entities.keys().map(g => ({
            molecularProfileId: entry.molecularProfileId,
            treatmentId: oncoprint.props.store.treatmentCache.get({
              treatmentId: g
            })!.data!.treatmentId,
            treatmentName: oncoprint.props.store.treatmentCache.get({
              treatmentId: g
            })!.data!.name
          }))
        )
      ...
      const tracks = cacheQueries.map(query => {
        ...
        const profile =
          molecularProfileIdToMolecularProfile[molecularProfileId];
        const dataCache = oncoprint.props.store.treatmentMolecularDataCache
          .result!;
...
export function makeGenesetHeatmapExpansionsMobxPromise(
  ...
  return remoteData<IGenesetExpansionMap>({
    ...
    invoke: async () => {
      ...
      const genesetGeneCache = oncoprint.props.store.genesetCorrelatedGeneCache
        .result!;
      ...
      expansionsByGenesetTrack.entries().forEach(([gsTrack, genes]) => {
        tracksByGenesetTrack[gsTrack] = genes.map(
          ({
            ...
            const data = dataCache.get({ entrezGeneId, molecularProfileId })!
              .data!;
            const profile =
              molecularProfileIdToMolecularProfile[molecularProfileId];
            return {
...
export function makeGenesetHeatmapTracksMobxPromise(
  ...
  return remoteData<IGenesetHeatmapTrackSpec[]>({
    ...
    invoke: async () => {
      const samples = oncoprint.props.store.samples.result!;
      const patients = oncoprint.props.store.patients.result!;
      const molecularProfile = oncoprint.props.store.genesetMolecularProfile
        .result!;
