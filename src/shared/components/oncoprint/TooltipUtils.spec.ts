...
describe("Oncoprint TooltipUtils", () => {
  describe("getCaseViewElt", () => {
    ...
    it("shows case id if single input and no linkout", () => {
      assert.equal(
        getCaseViewElt(
          [{ sample: "sampleID", patient: "patientID", study_id: "studyID" }],
          false
        ),
        "<span>sampleID</span>"
      );
    ...
    it("shows link if single input and linkout", () => {
      ...
      elt = $(
        getCaseViewElt([{ patient: "patientID", study_id: "studyID" }], true)
      );
    ...
    it("shows number if multiple input and no linkout", () => {
      assert.equal(
        getCaseViewElt(
          [
            { sample: "sampleID", patient: "patientID", study_id: "studyID" },
            { sample: "sampleID2", patient: "patientID2", study_id: "studyID" },
            { sample: "sampleID3", patient: "patientID3", study_id: "studyID" }
          ],
          false
        ),
        "<span>3 samples</span>"
  ...
  describe("makeGeneticTrackTooltip", () => {
    ...
    describe("custom driver annotations", () => {
      it("should show a binary custom driver icon with descriptive title, if theres a binary custom driver annotation", () => {
        ...
        assert.equal(
          tooltipOutput.find("img[title='Putative_Driver: annotation here']")
            .length,
          1
        );
        assert.equal(
      ...
      it("should show multiple binary custom driver icons with corresponding titles, if there are multiple annotated mutations", () => {
        ...
        let tooltipOutput = tooltip([datum]);
        assert.equal(
          tooltipOutput.find("img[title='Putative_Driver: annotation 1']")
            .length,
          1
        );
        assert.equal(
          tooltipOutput.find("img[title='Putative_Driver: annotation 2']")
            .length,
          1
        );
        assert.equal(
          tooltipOutput.find("img[title='Putative_Driver: 3 annotation']")
            .length,
          1
        );
        assert.equal(
    ...
    describe("profiled and not profiled", () => {
      it("should say 'Not profiled' if 'profiled_in' is empty and 'not_profiled_in' is not", () => {
        ...
        assert.isTrue(
          tooltipOutput
            .html()
            .indexOf("Not profiled in selected molecular profiles.") > -1
        );
        assert.equal(
      ...
      it("should say 'profiled' if 'not_profiled_in' is empty and 'profiled_in' is not", () => {
        ...
        assert.isTrue(
          tooltipOutput
            .html()
            .indexOf("Profiled in all selected molecular profiles.") > -1
        );
        assert.equal(
          tooltipOutput.html().match(/\(1\)/g),
          null,
      ...
      it("should give the correct spread of 'Not profiled' and 'Profiled' over multiple data", () => {
        ...
        assert.isTrue(
          tooltipOutput
            .html()
            .indexOf("Profiled in: Profile (2), Profile2 (2), Profile3 (1)") >
            -1,
        ...
        assert.isTrue(
          tooltipOutput
            .html()
            .indexOf("Profiled in: Profile (4), Profile2 (4), Profile3 (3)") >
            -1,
          "all the counts are correct (order is arbitrary)"
        );
        assert.isTrue(
    ...
    describe("gene panel information", () => {
      it("should show the correct gene panel entries in the tooltip, single and multiple data", () => {
        const datum1 = {
          ...
          na: true,
          profiled_in: [
            { molecularProfileId: "profile", genePanelId: "panel1" },
            { molecularProfileId: "profile", genePanelId: "panel2" },
            { molecularProfileId: "profile2", genePanelId: "panel3" }
          ],
          not_profiled_in: []
        };
        const datum2 = {
    ...
    describe("genetic alterations", () => {
        ...
        for (let i = -2; i <= 2; i++) {
          ...
          if (i === 0) {
            ...
            assert.equal(
              tooltipOutput
                .text()
                .match(new RegExp(`GENE ${disp_cna[i.toString()]}`, "g"))!
                .length,
              1
            );
            assert.equal(
      ...
      it("multiple alterations of same type across multiple cases - cna", () => {
        ...
        assert.equal(
          tooltipOutput.text().match(/GENE HOMODELETED\xa0\(3\)/g)!.length,
          1
        );
        assert.equal(
          tooltipOutput.text().match(/GENE HETLOSS\xa0\(3\)/g)!.length,
          1
        );
        assert.equal(
        ...
        assert.equal(
          tooltipOutput.text().match(/GENE HOMODELETED\xa0\(1\)/g)!.length,
          1
        );
        assert.equal(
          tooltipOutput.text().match(/GENE HETLOSS\xa0\(1\)/g)!.length,
          1
        );
        assert.equal(
      ...
      it("multiple alterations of different types across multiple cases", () => {
        ...
        assert.equal(tooltipOutput.text().match(/Fusion:/g)!.length, 1);
        assert.equal(
          tooltipOutput.text().match(/GENE fusion1\xa0\(3\)/g)!.length,
          1
        );
        assert.equal(
          tooltipOutput.text().match(/GENE fusion2\xa0\(4\)/g)!.length,
          1
        );
        ...
        assert.equal(
          tooltipOutput.text().match(/GENE AMPLIFIED\xa0\(3\)/g)!.length,
          1
        );
        assert.equal(
          tooltipOutput.text().match(/GENE GAIN\xa0\(1\)/g)!.length,
  ...
  describe("makeClinicalTrackTooltip", () => {
    describe("category track tooltip", () => {
      ...
      it("should show the given sample id", () => {
        const sampleTooltipResult = tooltip([
          { attr_val_counts: { a: 1 }, attr_val: "a", sample: "sampleID" }
        ]);
        assert.isTrue(
          sampleTooltipResult.html().indexOf("<span>sampleID</span>") > -1
        );
      });
      it("should show the given patient id", () => {
        ...
        assert.isTrue(
          patientTooltipResult.html().indexOf("<span>patientID</span>") > -1
        );
      });
      it("should show the correct output for a single value", () => {
        ...
        assert.isTrue(
          tooltipResult
            .html()
            .indexOf(
              `label1234: <span style="white-space:nowrap"><b>a</b></span>`
            ) > -1
        );
      });
      it("should show the correct output for multiple values", () => {
        const tooltipResult = tooltip([
          { attr_val_counts: { a: 1, b: 3 }, attr_val: "a", sample: "sampleID" }
        ]);
        assert.isTrue(
          tooltipResult
            .html()
            .indexOf(
              `label1234:<br><span style="white-space:nowrap"><b>a</b>: 1 sample</span><br><span style="white-space:nowrap"><b>b</b>: 3 samples</span>`
            ) > -1
        );
      });
      it("should show the correct output for multiple data, single value", () => {
        ...
        assert.isTrue(
          tooltipResult
            .html()
            .indexOf(
              `label1234: <span style="white-space:nowrap"><b>a</b> (4 samples)</span><br>`
            ) > -1
        );
      });
    ...
    describe("number track tooltip", () => {
      ...
      it("should show numerical data rounded to 2 decimal digits", () => {
        ...
        tooltipResult = tooltip([
          { attr_val_counts: { "0": 1 }, attr_val: "0", sample: "sampleID" }
        ]);
        assert.isTrue(
        ...
        // more than one data
        tooltipResult = tooltip([
          { attr_val_counts: { "0.13500013531": 1 }, sample: "sampleID" },
          { attr_val_counts: { "0.23500013531": 1 }, sample: "sampleID" },
          { attr_val_counts: { "0.33500013531": 1 }, sample: "sampleID" }
        ]);
        assert.isTrue(
          tooltipResult.html().indexOf("<b>0.24 (average of 3 values)</b>") >
            -1,
        ...
        assert.isTrue(
          tooltipResult.html().indexOf("<b>7.10 (average of 2 values)</b>") >
            -1,
        ...
        tooltipResult = tooltip([
          {
            attr_val_counts: { "-0.03500013531": 2, "-0.23500013531": 2 },
            sample: "sampleID"
          }
        ]);
        assert.isTrue(
          tooltipResult.html().indexOf("<b>-0.14 (average of 4 values)</b>") >
            -1,
          "correct result with no integer part, negative"
        );
        tooltipResult = tooltip([
        ...
        assert.isTrue(
          tooltipResult.html().indexOf("<b>-3.10 (average of 3 values)</b>") >
            -1,
          "correct result with integer part, negative"
        );
      });
