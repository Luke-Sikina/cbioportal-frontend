...
export default class MutationMapperDataStore
  ...
  public get selectedPositions() {
    return _.keyBy(
      findAllUniquePositions(this.selectionFilters).map(p => ({ position: p })),
      "position"
    );
  }
  ...
  public get highlightedPositions() {
    return _.keyBy(
      findAllUniquePositions(this.highlightFilters).map(p => ({ position: p })),
      "position"
    );
  }
  ...
  private clearPositionFilters() {
    ...
    this.selectionFilters = this.selectionFilters.filter(
  ...
  public dataSelectFilter(d: Mutation[]): boolean {
    return (
      ...
      !this.selectionFilters
        .map(dataFilter =>
          this.applyFilter(dataFilter, d[0], this.selectedPositions)
        )
        .includes(false)
    );
  }
  ...
  public dataHighlightFilter(d: Mutation[]): boolean {
    return (
      ...
      !this.highlightFilters
        .map(dataFilter =>
          this.applyFilter(dataFilter, d[0], this.highlightedPositions)
        )
        .includes(false)
    );
  }
