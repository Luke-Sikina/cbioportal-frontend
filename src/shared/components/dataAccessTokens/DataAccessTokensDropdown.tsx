...
function buildDataAccessTokenFileContents(
  ...
  if (!isNullOrUndefined(dat)) {
    const fileContents =
      ...
      "\n" +
      "creation_date: " +
      new Date(dat!.creationDate).toISOString() +
      "\n" +
      "expiration_date: " +
      new Date(dat!.expirationDate).toISOString();
    return fileContents;
  } else {
    alert(
    ...
    const listItems = [
      {
        id: "signout",
        action: (
          <a href={buildCBioPortalPageUrl(this.props.appStore.logoutUrl)}>
            Sign out
          </a>
        ),
        hide: false
      },
      {
        id: "datDownload",
        action: (
          <a onClick={() => this.downloadDataAccessTokenFile()}>
            Download token
          </a>
        ),
        hide:
