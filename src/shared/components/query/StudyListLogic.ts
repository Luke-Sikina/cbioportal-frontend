...
export default class StudyListLogic {
  ...
  cancerTypeContainsSelectedStudies(cancerType: CancerType): boolean {
    ...
    return (
      _.intersection(this.store.selectableSelectedStudies, descendantStudies)
        .length > 0
    );
  }
  ...
  isHighlighted(node: CancerTreeNode): boolean {
    return (
      !!this.store.searchText && !!this.map_node_filterBySearchText.get(node)
    );
  }
}
...
export class FilteredCancerTreeView {
  ...
  getSelectionReport() {
    ...
    let selectableSelectedStudies = selectableSelectedStudyIds.map(
      studyId =>
        this.store.treeData.map_studyId_cancerStudy.get(studyId) as CancerStudy
    );
