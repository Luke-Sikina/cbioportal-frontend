...
export default class QuickSearch extends React.Component {
      ...
      if (input.length > 0) {
        return Promise.all([
          client.getAllStudiesUsingGETWithHttpInfo({
            keyword: input,
            pageSize:
              DEFAULT_PAGE_SIZE + SHOW_MORE_SIZE * this.studyPageMultiplier
          }),
          client.getAllStudiesUsingGETWithHttpInfo({
            keyword: input,
            projection: "META"
          ...
          client.getAllGenesUsingGETWithHttpInfo({
            keyword: input,
            pageSize:
              DEFAULT_PAGE_SIZE + SHOW_MORE_SIZE * this.genePageMultiplier
          }),
          client.getAllGenesUsingGETWithHttpInfo({
            keyword: input,
            projection: "META"
          ...
          client.getAllPatientsUsingGETWithHttpInfo({
            keyword: input,
            pageSize:
              DEFAULT_PAGE_SIZE + SHOW_MORE_SIZE * this.patientPageMultiplier,
            projection: "DETAILED"
          }),
          client.getAllPatientsUsingGETWithHttpInfo({
            keyword: input,
          ...
          const geneCount = {
            value:
              parseInt(response[3].headers["total-count"]) - geneOptions.length,
            type: OptionType.GENE_COUNT
          };
          const patientOptions: any = response[4].body.map(
  ...
  private handleChange(newOption: any) {
    ...
    if (newOption.type === OptionType.STUDY) {
      ...
      const studyList =
        this.geneStudyQuery.isComplete && this.geneStudyQuery.result.query;
      parameters = {
        case_set_id: "all",
