...
> {
  ...
  private CancerTypeDescendantStudies(cancerList: CancerType[]) {
    return cancerList
      .filter(
        cancer =>
          cancer.cancerTypeId !== "other" && cancer.cancerTypeId !== "mixed"
      )
  ...
  public getExampleSection() {
    if (
      AppConfig.serverConfig.skin_right_nav_show_examples &&
      !_.isEmpty(AppConfig.serverConfig.skin_examples_right_column_html)
    ) {
      return (
        <div
      ...
      return (
        <div className="rightBarSection exampleQueries">
          ...
          <ul>
            ...
            <li>
              <Link
                to={`/results/${ResultsViewTab.MUTATIONS}?cancer_study_list=ov_tcga_pub&cancer_study_id=ov_tcga_pub&genetic_profile_ids_PROFILE_MUTATION_EXTENDED=ov_tcga_pub_mutations&Z_SCORE_THRESHOLD=2.0&case_set_id=ov_tcga_pub_3way_complete&case_ids=&gene_list=BRCA1+BRCA2&gene_set_choice=user-defined-list`}
              >
        ...
        {this.studyStore.cancerStudies.isComplete &&
          this.studyStore.cancerTypes.isComplete && (
            <div>
              <p>
                The portal contains{" "}
                {this.studyStore.cancerStudies.result.length} cancer studies{" "}
