...
import {
  ...
  TableCellStatus
} from "public-lib/components/TableCellStatus";
import {
  MyVariantInfo,
...
export default class GnomadColumnFormatter {
  ...
  private static getGnomadDataViz(
    ...
    if (genomeNexusCacheData == null) {
      ...
      const gnomadData = GnomadColumnFormatter.getData(
      ...
      if (gnomadData && (gnomadData.gnomadExome || gnomadData.gnomadGenome)) {
        ...
        gnomadUrl =
          gnomadData && gnomadData.dbsnp
            ? GnomadColumnFormatter.generateGnomadUrl(
                gnomadData.dbsnp.chrom,
                gnomadData.dbsnp.hg19.start,
                gnomadData.dbsnp.ref,
                gnomadData.dbsnp.alt
              )
            : "";
        ...
        if (result["Total"].alleleFrequency === 0) {
          ...
          display = (
            <span>
              {parseFloat(
                result["Total"].alleleFrequency.toString()
              ).toExponential(1)}
            </span>
          );
        }
  ...
  public static setGnomadTableData(
    key: string,
    data: Gnomad,
    result: { [key: string]: GnomadData }
  ) {
    ...
    const alleleCountName: keyof AlleleCount = (PopulationName[key]
      ? ColumnName.alleleCount + "_" + PopulationName[key]
      : ColumnName.alleleCount
    ).toString() as keyof AlleleCount;
    const alleleNumberName: keyof AlleleNumber = (PopulationName[key]
      ? ColumnName.alleleNumber + "_" + PopulationName[key]
      : ColumnName.alleleNumber
    ).toString() as keyof AlleleNumber;
    const homozygotesName: keyof Homozygotes = (PopulationName[key]
      ? ColumnName.homozygotes + "_" + PopulationName[key]
      : ColumnName.homozygotes
    ).toString() as keyof Homozygotes;
    const alleleFrequencyName: keyof AlleleFrequency = (PopulationName[key]
      ? ColumnName.alleleFrequency + "_" + PopulationName[key]
      : ColumnName.alleleFrequency
  ...
  public static calculateAlleleFrequency(
    count: number | null,
  ...
  public static generateGnomadUrl(
    chrom: String | null,
    start: number | null,
    alt: String | null,
    ref: String | null
