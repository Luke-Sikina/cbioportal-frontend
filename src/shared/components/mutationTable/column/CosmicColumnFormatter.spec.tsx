...
describe("CosmicColumnFormatter", () => {
  ...
  it("generates component tooltip", () => {
    assert.isTrue(
      component273.find(DefaultTooltip).exists(),
      "Tooltip should exists for TP53 R273 missense mutation"
    );
    assert.isTrue(
      component38.find(DefaultTooltip).exists(),
      "Tooltip should exists for PIK3CA R38 missense mutation"
    );
    assert.isFalse(
      component666.find(DefaultTooltip).exists(),
  ...
  it("sets component cell value property", () => {
    assert.equal(
      CosmicColumnFormatter.getSortValue([mutation273], cosmicData),
      2 + 3 + 5,
      "Sort value for TP53 R273 missense mutation is correct"
    );
    assert.equal(
      CosmicColumnFormatter.getSortValue([mutation38], cosmicData),
      7 + 11 + 13,
      "Sort value value property for PIK3CA R38 missense mutation is correct"
    );
    assert.equal(
      CosmicColumnFormatter.getSortValue([mutation666], cosmicData),
      null,
