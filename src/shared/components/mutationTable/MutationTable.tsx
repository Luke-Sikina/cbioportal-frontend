...
> extends React.Component<P, {}> {
  ...
  protected generateColumns() {
    ...
    this._columns[MutationTableColumnType.COHORT] = {
      ...
      render: (d: Mutation[]) =>
        this.props.variantCountCache ? (
          CohortColumnFormatter.renderFunction(d, this.props.mutSigData, this
            .props.variantCountCache as VariantCountCache)
        ) : (
          <span></span>
        ),
      sortBy: (d: Mutation[]) => {
    ...
    this._columns[MutationTableColumnType.NUM_MUTATIONS] = {
      ...
      tooltip: (
        <span>Total number of nonsynonymous mutations in the sample</span>
      ),
      align: "right"
    };
    ...
    this._columns[MutationTableColumnType.GNOMAD] = {
      ...
      tooltip: (
        <span>
          <a href="https://gnomad.broadinstitute.org/">gnomAD</a> population
          allele frequencies. Overall population allele frequency is shown.
          Hover over a frequency to see the frequency for each specific
          population.
        </span>
      ),
      defaultSortDirection: "desc",
    ...
    this._columns[MutationTableColumnType.DBSNP] = {
      ...
      tooltip: (
        <span style={{ maxWidth: 370, display: "block", textAlign: "left" }}>
          ...
          different species.
          <br />
          NOTE: Currently only SNPs, single base deletions and insertions are
          supported.
        </span>
      ),
      defaultSortDirection: "desc",
