...
const MIN_NUMBER_OF_MUTATIONS_STATEMENT =
  "There are not enough mutations to call mutational signatures for this case.";
const DUBIOUS_NUMBER_OF_MUTATIONS_THRESHOLD = 15;
const DUBIOUS_NUMBER_OF_MUTATIONS_STATEMENT =
  "This case has a low number of mutations, signatures are highly uncertain.";
...
export function prepareMutationalSignaturesForHeader(
  mutationalSignatureData: IMutationalSignature[],
  mutationalSignatureMetaData: IMutationalSignatureMeta[],
  ...
  for (const mutationalSignatureSample of mutationalSignatureDataForUniqueSample) {
    ...
    significantMutationalSignatureForSample.numberOfMutationsForSample =
      mutationalSignatureSample.numberOfMutationsForSample;

    if (mutationalSignatureSample.confidence > 0.85) {
      ...
      if (
        significantMutationalSignatureForSample.numberOfMutationsForSample <=
        MIN_NUMBER_OF_MUTATIONS_THRESHOLD
      ) {
        //account for low number of mutations
        significantMutationalSignatureForSample.confidenceStatement += MIN_NUMBER_OF_MUTATIONS_STATEMENT;
      } else if (
        significantMutationalSignatureForSample.numberOfMutationsForSample <=
        DUBIOUS_NUMBER_OF_MUTATIONS_THRESHOLD
      ) {
        significantMutationalSignatureForSample.confidenceStatement += DUBIOUS_NUMBER_OF_MUTATIONS_STATEMENT;
      }

      for (const mutationalSignature of mutationalSignatureMetaData) {
        //generate confidence statement depending on the mutational signature
        if (
          mutationalSignatureSample.mutationalSignatureId ===
          mutationalSignature.mutationalSignatureId
        ) {
          significantMutationalSignatureForSample.confidenceStatement +=
            " " + mutationalSignature.confidenceStatement;
        }
      }
    }
  }
