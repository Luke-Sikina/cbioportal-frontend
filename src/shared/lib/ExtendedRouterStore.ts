...
export default class ExtendedRouterStore extends RouterStore {
  ...
  public get urlLengthThresholdForSession(): number {
    return (
      this._urlLengthThresholdForSession ||
      parseInt(AppConfig.serverConfig.session_url_length_threshold, 10)
    );
  }
  ...
  @computed get needsRemoteSessionLookup() {
    ...
    const needsRemoteSessionLookup =
      this.session_id !== undefined &&
      this.session_id !== "pending" &&
  ...
  remoteSessionData = remoteData({
    ...
    onResult: () => {
      if (this.remoteSessionData.result) {
        ...
        if (this._session && this._session.id !== "pending")
  ...
  @action updateRoute(
    ...
    if (
      ...
      !this.sessionEnabledForPath(path) ||
      JSON.stringify(newQuery).length < this.urlLengthThresholdForSession
    ) {
      ...
      if (
        !this._session ||
        !_.isEqual(this._session.query, newQuery) ||
        _.size(newParams) > 0
      ) {
        ...
        this.saveRemoteSession(pendingSession.query).then(sessionResponse => {
          ...
          this.replace(
            URL.format({
              ...
              query: { session_id: sessionResponse.id },
              hash: this.location.hash
            })
          );
        });
      } else {
        // we already have a session but we only need to update path or hash
        this[replace ? "replace" : "push"](
          URL.format({
            pathname: path,
            query: { session_id: this._session.id },
            hash: this.location.hash
          })
        );
      }
